...
export default function MyLearningPage() {
  ...
  const { t } = useI18n();
  const tm = t.training.myLearningPage;
    ...
    <div key={assignment.id} className="bg-white rounded-xl border border-gray-200 p-4 hover:shadow-md transition-shadow">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {assignment.deadline && <p className="text-xs text-gray-400 mt-1">{tm.deadline}: {new Date(assignment.deadline).toLocaleDateString('fr-FR')}</p>}
        ...
        <div className="flex flex-col gap-1">
          {assignment.status === 'assigned' && (
            <button onClick={() => startAssignment(assignment.id)} className="px-3 py-1.5 bg-primary-500 text-white text-xs font-medium rounded-lg hover:bg-primary-600 flex items-center gap-1"><Play className="w-3 h-3" />{tm.start}</button>
          )}
          {assignment.status === 'in_progress' && (
            <>
              {assignment.course_external_url && <a href={assignment.course_external_url} target="_blank" rel="noopener noreferrer" className="px-3 py-1.5 bg-primary-500 text-white text-xs font-medium rounded-lg hover:bg-primary-600 flex items-center gap-1"><ExternalLink className="w-3 h-3" />{tm.access}</a>}
              <button onClick={() => openCompleteModal(assignment)} className="px-3 py-1.5 bg-green-500 text-white text-xs font-medium rounded-lg hover:bg-green-600 flex items-center gap-1"><CheckCircle className="w-3 h-3" />{tm.finish}</button>
            </>
          )}
          {assignment.status === 'pending_validation' && (
            <button onClick={() => openCompleteModal(assignment)} className="px-3 py-1.5 bg-orange-500 text-white text-xs font-medium rounded-lg hover:bg-orange-600 flex items-center gap-1"><Edit className="w-3 h-3" />{tm.modify}</button>
          )}
          {assignment.status === 'rejected' && (
            <button onClick={() => openCompleteModal(assignment)} className="px-3 py-1.5 bg-red-500 text-white text-xs font-medium rounded-lg hover:bg-red-600 flex items-center gap-1"><RefreshCw className="w-3 h-3" />{tm.resubmit}</button>
          )}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs text-gray-500">{tm.toStart}</p>
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs text-gray-500">{tm.inProgress}</p>
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs text-gray-500">{tm.completed}</p>
        ...
        <div className={`rounded-xl p-4 shadow-sm border ${pendingValidation.length > 0 ? 'bg-amber-50 border-amber-200' : 'bg-white border-gray-100'}`}>
          <div className="flex items-center justify-between">
            <div>
              <p className={`text-xs ${pendingValidation.length > 0 ? 'text-amber-600' : 'text-gray-500'}`}>{tm.pending}</p>
      ...
      {hasPermission(userRole, 'validate_completion') && pendingValidations.length > 0 && (
        <div className="mt-8">
          <h3 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2"><AlertTriangle className="w-5 h-5 text-amber-600" />{tm.validationsPending} ({pendingValidations.length})</h3>
          <div className="space-y-3">{pendingValidations.map((a) => (
            <div key={a.id} className="bg-white rounded-xl border border-amber-200 p-4 flex items-center justify-between">
              ...
              <button onClick={() => { setSelectedAssignment(a); setShowValidationModal(true); }} className="px-4 py-2 bg-amber-500 text-white text-sm font-medium rounded-lg hover:bg-amber-600">{tm.validate}</button>
            </div>
          ))}</div>
        </div>
      )}
