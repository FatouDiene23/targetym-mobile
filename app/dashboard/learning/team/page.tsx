          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
              <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs text-gray-500">{t.training.teamPage.members}</p>
              ...
              <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs text-gray-500">{t.training.teamPage.inProgress}</p>
              ...
              <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs text-gray-500">{t.training.teamPage.completed}</p>
              ...
              <div className={`rounded-xl p-4 shadow-sm border ${pendingValCount > 0 ? 'bg-amber-50 border-amber-200' : 'bg-white border-gray-100'}`}>
                <div className="flex items-center justify-between">
                  <div>
                    <p className={`text-xs ${pendingValCount > 0 ? 'text-amber-600' : 'text-gray-500'}`}>{t.training.teamPage.toValidate}</p>
            ...
            {teamAssignments.length === 0 ? (
              <div className="bg-white rounded-xl p-12 text-center">
                ...
                <p className="text-gray-500">{t.training.teamPage.noTeamTraining}</p>
              </div>
            ) : (
                    ...
                    <div key={emp.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                      <div className="p-4 bg-gray-50 border-b border-gray-100 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-4 text-sm">
                          {inProgress > 0 && <span className="px-2 py-1 bg-blue-100 text-blue-700 rounded-full">{inProgress} {t.training.teamPage.inProgressBadge}</span>}
                          {pending > 0 && <span className="px-2 py-1 bg-amber-100 text-amber-700 rounded-full">{pending} {t.training.teamPage.toValidateBadge}</span>}
                          {completed > 0 && <span className="px-2 py-1 bg-green-100 text-green-700 rounded-full">{completed} {t.training.teamPage.completedBadge}</span>}
                        </div>
                      </div>
                          ...
                          <div key={a.id} className={`flex items-center justify-between p-3 rounded-lg ${a.status === 'pending_validation' ? 'bg-amber-50' : a.status === 'completed' ? 'bg-green-50' : a.status === 'rejected' ? 'bg-red-50' : 'bg-gray-50'}`}>
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-xs text-gray-500">{a.course_duration}h • {t.training.teamPage.deadlineLabel}: {a.deadline || t.training.teamPage.notDefined}</p>
                              </div>
                            </div>
                            <div className="flex items-center gap-2">
                              ...
                              {a.status === 'pending_validation' && (
                                <button onClick={() => { setSelectedAssignment(a); setShowValidationModal(true); }} className="px-3 py-1 bg-amber-500 text-white text-xs font-medium rounded-lg hover:bg-amber-600">{t.training.teamPage.validateBtn}</button>
                              )}
                            </div>
                          </div>
                        ))}
