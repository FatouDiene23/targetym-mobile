...
const EVENT_CONFIGS: Record<EventType, { label: string; color: string; bgColor: string; borderColor: string; dotColor: string }> = {
  ...
  holiday:         { label: 'Jours fériés',            color: 'text-gray-700',    bgColor: 'bg-gray-50',     borderColor: 'border-gray-200',   dotColor: 'bg-gray-500' },
};
...
function getPublicHolidays(year: number): { date: string; name: string }[] {
  return [
    { date: `${year}-01-01`, name: 'Jour de l\'An' },
    { date: `${year}-04-04`, name: 'Fête de l\'Indépendance' },
...
function buildCalendarEvents(
  ...
  myLeaves
    ...
    .forEach(leave => {
      ...
      events.push({
        id: `leave-${leave.id}`,
        title: leave.leave_type_name || 'Congé',
        subtitle: leave.status === 'pending' ? 'En attente' : 'Approuvé',
        ...
        allDay: true,
        details: `${leave.days_requested} jour(s)${leave.reason ? ` — ${leave.reason}` : ''}`,
      });
    });
    ...
    teamLeaves
      ...
      .forEach(leave => {
        ...
        events.push({
          ...
          title: `${leave.employee_name || 'Collaborateur'} — ${leave.leave_type_name || 'Congé'}`,
          date: leave.start_date,
          endDate: leave.end_date,
          type: 'team_leave',
          color: config.color,
          ...
          allDay: true,
          details: `${leave.days_requested} jour(s)`,
        });
      });
  }
  ...
  relevantEmployees.forEach(emp => {
    ...
    events.push({
      ...
      title: `🎉 ${emp.first_name} ${emp.last_name}`,
      subtitle: `${yearsOfService} an(s) dans l'entreprise`,
  ...
  if (isManager || ['rh', 'admin', 'dg'].includes(userRole)) {
    ...
    targetEmployees.forEach(emp => {
      ...
      events.push({
        ...
        allDay: true,
        details: 'Période d\'essai à évaluer',
      });
    });
  }
        ...
        <div className="relative bg-white rounded-xl shadow-xl w-full max-w-sm p-6">
          ...
          {event.endDate && event.endDate !== event.date && (
            <div className="flex items-center gap-2 text-sm text-gray-600 mb-2">
              ...
              <span>Jusqu&apos;au {new Date(event.endDate).toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</span>
            </div>
          )}
          {event.details && (
          ...
          <div key={dateStr} className={`bg-white rounded-xl border ${isToday ? 'border-primary-300 ring-1 ring-primary-100' : 'border-gray-200'} overflow-hidden`}>
            <div className={`flex items-center gap-3 px-5 py-3 border-b ${isToday ? 'bg-primary-50 border-primary-200' : 'bg-gray-50 border-gray-100'}`}>
              ...
              <div>
                ...
                {isToday && <span className="ml-2 text-xs bg-primary-100 text-primary-700 px-2 py-0.5 rounded-full font-medium">Aujourd&apos;hui</span>}
              </div>
              <span className="ml-auto text-xs text-gray-400 font-medium">{dayEvents.length} événement(s)</span>
            </div>
    ...
    <>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {showFilters && (
          <div className="bg-white rounded-xl border border-gray-200 p-4 mb-6">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold text-gray-700">Filtrer par type d&apos;événement</h3>
        ...
        <div className="mt-6 bg-white rounded-xl border border-gray-200 p-4">
          <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Légende</h3>
