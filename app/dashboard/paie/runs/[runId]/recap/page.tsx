...
function SlipLinesModal({
  ...
  const handlePrint = () => {
    (async () => {
      ...
      if (!win) { toast.error(t.payroll.recap.popupBlocked); return; }
  ...
  <div class="page">
    ...
    <div class="no-print">
      <button class="btn-print" onclick="window.print();">Imprimer / Sauvegarder en PDF</button>
        ...
        win.close();
        toast.error(t.payroll.recap.bulletinGenerationError);
      }
    })();
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between p-5 border-b sticky top-0 bg-white">
          <div>
            ...
            <p className="text-xs text-gray-400">{t.payroll.recap.payslipTitle} — {MONTHS_FR[run.period_month - 1]} {run.period_year}</p>
          </div>
          <div className="flex items-center gap-2">
            <button
              ...
              title={t.payroll.recap.printExportPdf}
            >
              <Printer className="w-4 h-4" />
              {t.payroll.recap.print}
            </button>
        ...
        <div className="p-5 space-y-2">
          <LineTable lines={gains} title={t.payroll.recap.gains} colorClass="text-green-600" />
          <LineTable lines={deductions} title={t.payroll.recap.employeeDeductions} colorClass="text-red-600" />
          <LineTable lines={patronal} title={t.payroll.recap.employerContributions} colorClass="text-orange-600" />

          {/* Récapitulatif */}
          <div className="bg-gray-50 rounded-xl p-4 text-sm space-y-1.5">
            <div className="flex justify-between">
              <span className="text-gray-500">{t.payroll.recap.grossSalary}</span>
              <span className="font-semibold">{formatXOF(slip.brut_total)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">{t.payroll.recap.employeeContributionsFull}</span>
              <span className="font-semibold text-red-600">- {formatXOF(slip.cotisations_salariales)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">{t.payroll.recap.ir}</span>
              <span className="font-semibold text-red-600">- {formatXOF(slip.ir_amount)}</span>
            </div>
            <div className="border-t my-1" />
            <div className="flex justify-between text-base">
              <span className="font-bold text-gray-800">{t.payroll.recap.netToPay}</span>
              <span className="font-bold text-green-700">{formatXOF(slip.net_a_payer)}</span>
            </div>
            <div className="flex justify-between text-xs text-gray-400 pt-1">
              <span>{t.payroll.recap.employerCharges}</span>
...
export default function RecapPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      toast.error(t.payroll.recap.loadError);
    } finally {
      setLoading(false);
    }
  }, [runIdNum]);
  ...
  const openDetail = async (slip: PaySlip) => {
    ...
    try {
      ...
      setDetailSlip(full);
    } catch { toast.error(t.payroll.recap.bulletinLoadError); }
  ...
  const handleSimulate = () => {
    ...
    setConfirmDialog({
      open: true,
      title: t.payroll.recap.launchSimulation,
      message: t.payroll.recap.launchSimulationMessage,
      onConfirm: async () => {
        setActionLoading(true);
        try {
          const updated = await simulateRun(run.id);
          toast.success(t.payroll.recap.simulationDone);
          setRun(updated);
          const slipsData = await getSlips(runIdNum);
          setSlips(slipsData);
        } catch (err: unknown) {
    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          toast.success(t.payroll.recap.validatedMessage);
          setRun(updated);
        } catch (err: unknown) {
    ...
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <main className="flex-1 p-6 max-w-6xl mx-auto w-full">
        ...
        <div className="flex items-center gap-2 text-sm text-gray-400 mb-4">
          ...
          <Link href="/dashboard/paie/runs" className="hover:text-primary-600">{t.payroll.runs}</Link>
          <ChevronDown className="w-3 h-3 rotate-[-90deg]" />
          <span className="text-gray-700 font-medium">
            {t.payroll.recap.breadcrumbRecap} — {run ? `${MONTHS_FR[run.period_month - 1]} ${run.period_year}` : '…'}
          </span>
        </div>

        {loading ? (
          <div className="flex items-center justify-center h-64 text-gray-400">
            <Loader2 className="w-6 h-6 animate-spin mr-2" /> {t.payroll.recap.loading}
          </div>
        ) : !run ? (
          <div className="text-center text-gray-400 py-20">{t.payroll.recap.runNotFound}</div>
        ) : (
          <>
            {/* En-tête */}
            <div className="flex items-center justify-between mb-6">
              <div>
                <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
                  ...
                  {t.payroll.recap.title} — {MONTHS_FR[run.period_month - 1]} {run.period_year}
                </h1>
                <div className="flex items-center gap-3 mt-1">
                  ...
                  {run.employee_count != null && (
                    <span className="text-sm text-gray-400">{run.employee_count > 1 ? t.payroll.recap.employeeCountPlural.replace('{count}', String(run.employee_count)) : t.payroll.recap.employeeCount.replace('{count}', String(run.employee_count))}</span>
                  )}
                </div>
              </div>
              <div className="flex items-center gap-3">
                ...
                {(status === 'draft' || status === 'simulation') && (
                  ...
                  >
                    ...
                    {t.payroll.recap.simulateAction}
                  </button>
                )}
                {status === 'simulation' && (
                  <button
                  ...
                  >
                    ...
                    {t.payroll.recap.validateAction}
                  </button>
                )}
              </div>
            </div>
            ...
            {(status === 'simulation' || status === 'validated') && (
              <div className="grid grid-cols-4 gap-4 mb-6">
                <StatCard
                  label={t.payroll.recap.grossPayroll}
                  value={formatXOF(run.total_brut)}
                  icon={TrendingUp}
                  color="bg-primary-50 text-primary-600"
                />
                <StatCard
                  label={t.payroll.recap.totalNetToPay}
                  value={formatXOF(run.total_net)}
                  icon={Wallet}
                  color="bg-green-50 text-green-600"
                />
                <StatCard
                  label={t.payroll.recap.employerCharges}
                  value={formatXOF(run.total_charges_patronales)}
                  icon={TrendingDown}
                  color="bg-orange-50 text-orange-600"
                />
                <StatCard
                  label={t.payroll.recap.employee}
            ...
            {(status === 'simulation' || status === 'validated') && slips.length > 0 && (
              <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
                <div className="px-4 py-3 border-b bg-gray-50">
                  <h2 className="font-medium text-gray-700 text-sm">{t.payroll.recap.payslipsTitle}</h2>
                </div>
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 border-b border-gray-100">
                    <tr>
                      <th className="text-left px-4 py-3 font-medium text-gray-500">{t.payroll.recap.employee}</th>
                      <th className="text-right px-4 py-3 font-medium text-gray-500">{t.payroll.recap.gross}</th>
                      <th className="text-right px-4 py-3 font-medium text-gray-500">{t.payroll.recap.employeeContributions}</th>
                      <th className="text-right px-4 py-3 font-medium text-gray-500">{t.payroll.recap.ir}</th>
                      <th className="text-right px-4 py-3 font-medium text-gray-500">{t.payroll.recap.netToPay}</th>
                      <th className="text-right px-4 py-3 font-medium text-gray-500">{t.payroll.recap.employerChargesShort}</th>
            ...
            {status === 'draft' && (
              <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center text-gray-400">
                ...
                <p className="font-medium">{t.payroll.recap.notSimulated}</p>
                <p className="text-sm mt-1">
                  {t.payroll.recap.notSimulatedHint}
                </p>
              </div>
            )}
          </>
