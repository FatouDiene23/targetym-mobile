...
function VariableModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (employeeId === 0) return toast.error(t.payroll.variables.selectEmployeeError);
    if (componentId === 0) return toast.error(t.payroll.variables.selectComponentError);
    setSaving(true);
    try {
      ...
      toast.success(variable ? t.payroll.variables.variableModified : t.payroll.variables.variableAdded);
      onSaved(result);
    } catch (err: unknown) {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        <div className="flex items-center justify-between p-5 border-b">
          <h2 className="font-semibold text-gray-800 text-lg">
            {variable ? t.payroll.variables.editVariable : t.payroll.variables.addVariable}
          </h2>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          {!variable && (
            <>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t.payroll.variables.selectEmployee}</label>
                <select
                  ...
                  required
                >
                  <option value={0}>{t.payroll.variables.selectPlaceholder}</option>
                  {employees.map(emp => (
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t.payroll.variables.selectComponent}</label>
                {manualComponents.length === 0 ? (
                  <p className="text-xs text-orange-600 flex items-center gap-1">
                    <AlertCircle className="w-3 h-3" />
                    {t.payroll.variables.noManualComponent}
                  </p>
                ) : (
                  <select
                    className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
          ...
          {variable && (
            <div className="text-sm text-gray-600 bg-gray-50 rounded-lg p-3">
              <div><span className="text-gray-400">{t.payroll.variables.employeeLabel}</span> {variable.component_name}</div>
              <div><span className="text-gray-400">{t.payroll.variables.componentLabel}</span> {variable.component_code}</div>
            </div>
          )}
          <div>
