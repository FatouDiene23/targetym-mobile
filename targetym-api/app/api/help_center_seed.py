"""
Endpoint API pour seeder le centre d'aide
...
@router.post("/seed", dependencies=[Depends(require_super_admin)])
def seed_help_center(db: Session = Depends(get_db)) -> Dict[str, str]:
    """
    Peupler le centre d'aide avec des articles utiles
    Accessible uniquement aux SUPER_ADMIN
    """
    try:
...
Voici des exemples concrets que vous pouvez poser à l'assistant IA.
...
**Astuce** : N'hésitez pas à poser des questions de suivi !
""",
                "is_published": True,
                "order": 2
            }
        ...
        return {
            "status": "success",
            "message": f"Centre d'aide peuplé avec succès : {len(categories)} catégories et {len(articles_data)} articles créés"
        }
        
    except Exception as e:
        db.rollback()
        print(f"[help_center_seed] ERREUR lors du seed: {str(e)}")
        import traceback
        traceback.print_exc()
