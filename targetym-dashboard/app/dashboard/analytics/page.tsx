...
function getSectionHeaders(t: any): Record<string, { title: string; subtitle: string }> {
  return {
    'overview':         { title: t.analytics.sections.overview.title,        subtitle: t.analytics.sections.overview.subtitle },
    'effectif':         { title: t.analytics.sections.effectif.title,        subtitle: t.analytics.sections.effectif.subtitle },
    'performance':      { title: t.analytics.sections.performance.title,     subtitle: t.analytics.sections.performance.subtitle },
    'talents':          { title: t.analytics.sections.talents.title,         subtitle: t.analytics.sections.talents.subtitle },
    'formation':        { title: t.analytics.sections.formation.title,       subtitle: t.analytics.sections.formation.subtitle },
    'engagement':       { title: t.analytics.sections.engagement.title,      subtitle: t.analytics.sections.engagement.subtitle },
    'recrutement':      { title: t.analytics.sections.recrutement.title,     subtitle: t.analytics.sections.recrutement.subtitle },
    'masse-salariale':  { title: t.analytics.sections.masseSalariale.title,  subtitle: t.analytics.sections.masseSalariale.subtitle },
    'impact-formation': { title: t.analytics.sections.impactFormation.title, subtitle: t.analytics.sections.impactFormation.subtitle },
  };
}
...
export default function PeopleAnalyticsPage() {
  ...
  const SECTION_HEADERS = getSectionHeaders(t);
  ...
  const renderOverview = () => {
    ...
    const perfSubtitle = perfScore
      ? (okrAvg ? `${okrAvg}% OKR` : `${perfOverview.total_evals} ${t.analytics.evaluations}`)
      : t.analytics.noEvaluation;
    ...
    const talentsSubtitle = totalTalents > 0
      ? `${stars} ${t.analytics.starsLabel} • ${highPot} ${t.analytics.highPotentials}`
      : t.analytics.noNineBoxPlacement;
    ...
    if (nbAtRisk > 0) {
      alertesIA.push({ type: "critical", icon: "AlertTriangle", title: t.analytics.collaboratorsAtRisk, message: t.analytics.collaboratorsAtRiskMessage.replace("{count}", String(nbAtRisk)), action: t.analytics.viewTalents, tab: 3 });
    }
    if (overview && overview.turnover >= 10) {
      alertesIA.push({ type: "warning", icon: "TrendingDown", title: t.analytics.highTurnover, message: t.analytics.highTurnoverMessage.replace("{rate}", String(overview.turnover)), action: t.analytics.analyze, tab: 1 });
    }
    if (overview && overview.absenteeism > 5) {
      alertesIA.push({ type: "warning", icon: "Clock", title: t.analytics.highAbsenteeism, message: t.analytics.highAbsenteeismMessage.replace("{rate}", String(overview.absenteeism)), action: t.analytics.viewDetails, tab: 1 });
    }
    if (perfOverview?.okr?.at_risk > 0) {
      alertesIA.push({ type: "warning", icon: "AlertTriangle", title: t.analytics.okrAtRisk, message: t.analytics.okrAtRiskMessage.replace("{count}", String(perfOverview.okr.at_risk)), action: t.analytics.viewOkr, tab: 2 });
    }
    if (perfOverview?.top_performers > 0) {
      alertesIA.push({ type: "info", icon: "Award", title: t.analytics.excellenceDetected, message: t.analytics.excellenceDetectedMessage.replace("{count}", String(perfOverview.top_performers)), action: t.analytics.details, tab: 2 });
    }
    if (perfOverview?.okr?.on_track > 0) {
      alertesIA.push({ type: "info", icon: "TrendingUp", title: t.analytics.okrGoodProgress, message: t.analytics.okrGoodProgressMessage.replace("{count}", String(perfOverview.okr.on_track)).replace("{total}", String(perfOverview.okr.total)), action: t.analytics.viewOkr, tab: 2 });
    }
    ...
    <div className="space-y-6">
      ...
      <div data-tour="analytics-kpis" className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {renderKPICard(
          t.analytics.totalEffectif,
          overview?.total_employees ?? "—",
          <Users size={20} className="text-primary-600" />,
          "bg-primary-50",
          `${overview?.entries ?? 0} ${t.analytics.entries.toLowerCase()} / ${overview?.exits ?? 0} ${t.analytics.exits.toLowerCase()}`,
          () => setActiveTab(1)
        )}
        {renderKPICard(
          ...
          "bg-red-50",
          `${t.analytics.retention}: ${overview?.retention ?? "—"}%`,
          () => setActiveTab(1)
        )}
        {renderKPICard(
          t.analytics.performanceAvg,
        ...
        {renderKPICard(
          ...
          "bg-orange-50",
          t.analytics.onPeriod,
          () => setActiveTab(5)
        )}
        {renderKPICard(
          t.analytics.masseSalariale,
      ...
      <div data-tour="analytics-charts" className="bg-white rounded-xl border p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="font-semibold text-gray-900">{t.analytics.alertsAndRecommendations}</h3>
                  </div>
        {alertesIA.length === 0 ? (
          <p className="text-sm text-gray-400 text-center py-4">{t.analytics.noAlert}</p>
        ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-gray-900">{t.analytics.staffEvolution}</h3>
                      </div>
        ...
        <div className="bg-white rounded-xl border p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-gray-900">{t.analytics.performanceByTeam}</h3>
          </div>
          {byDept.length > 0 ? (
            <ResponsiveContainer width="100%" height={250}>
            ...
            <div className="flex flex-col items-center justify-center h-[250px] text-gray-400">
              ...
              <p className="text-sm">{t.analytics.noEvalSubmitted}</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: t.analytics.turnover, value: `${overview?.turnover ?? "—"}%`, gradient: "from-red-500 to-orange-500", icon: <TrendingDown size={24} /> },
          { label: t.analytics.retention, value: `${overview?.retention ?? "—"}%`, gradient: "from-green-500 to-emerald-500", icon: <Shield size={24} /> },
          { label: t.analytics.parityHF, value: `${overview?.pct_hommes ?? "—"}/${overview?.pct_femmes ?? "—"}`, gradient: "from-primary-500 to-indigo-500", icon: <Users size={24} /> },
          { label: t.analytics.absenteeism, value: `${overview?.absenteeism ?? "—"}%`, gradient: "from-amber-500 to-yellow-500", icon: <Clock size={24} /> },
        ].map((m, i) => (
  ...
  const renderEffectif = () => (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mt-3">
        {renderKPICard(t.analytics.totalEffectif, overview?.total_employees ?? "—", <Users size={20} className="text-primary-600" />, "bg-primary-50")}
        {renderKPICard(t.analytics.entries, overview?.entries ?? "—", <UserPlus size={20} className="text-green-600" />, "bg-green-50", t.analytics.onPeriod)}
        {renderKPICard(t.analytics.exits, overview?.exits ?? "—", <UserMinus size={20} className="text-red-600" />, "bg-red-50", t.analytics.onPeriod)}
        {renderKPICard(t.analytics.turnover, `${overview?.turnover ?? "—"}%`, <TrendingDown size={20} className="text-orange-600" />, "bg-orange-50")}
        {renderKPICard(t.analytics.avgTenure, `${overview?.avg_tenure ?? "—"} ${t.analytics.years}`, <Clock size={20} className="text-purple-600" />, "bg-purple-50")}
        {renderKPICard(t.analytics.parityHF, `${overview?.pct_hommes ?? "—"}% / ${overview?.pct_femmes ?? "—"}%`, <Users size={20} className="text-indigo-600" />, "bg-indigo-50")}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border p-6">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <AreaChart data={effectifsEvolution}>
              ...
              <Tooltip />
              <Legend />
              <Area type="monotone" dataKey="effectif" name={t.analytics.effectif} stroke="#066C6C" fill="#066C6C20" strokeWidth={2} />
              <Area type="monotone" dataKey="entrees" name={t.analytics.entries} stroke="#10b981" fill="#10b98120" strokeWidth={2} />
              <Area type="monotone" dataKey="sorties" name={t.analytics.exits} stroke="#ef4444" fill="#ef444420" strokeWidth={2} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="bg-white rounded-xl border p-6">
          <h3 className="font-semibold text-gray-900 mb-4">{t.analytics.deptDistribution}</h3>
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie
        ...
        {/* Pyramide des âges */}
        <div className="bg-white rounded-xl border p-6">
          <h3 className="font-semibold text-gray-900 mb-4">{t.analytics.agePyramid}</h3>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={pyramideAges}>
              ...
              <Tooltip />
              <Legend />
              <Bar dataKey="hommes" name={t.analytics.men} fill="#066C6C" radius={[4, 4, 0, 0]} />
              <Bar dataKey="femmes" name={t.analytics.women} fill="#ec4899" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="bg-white rounded-xl border p-6">
          <h3 className="font-semibold text-gray-900 mb-4">{t.analytics.turnoverByDept}</h3>
          <ResponsiveContainer width="100%" height={300}>
  ...
  const renderPerformance = () => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-3">
        {renderKPICard(
          t.analytics.okrAvgProgress,
          okr ? `${okr.avg_progress}%` : "—",
          <Target size={20} className="text-green-600" />,
          "bg-green-50",
          okr ? `${okr.on_track} ${t.analytics.onTrack} • ${okr.at_risk} ${t.analytics.atRiskLabel}` : undefined
        )}
        {renderKPICard(
          t.analytics.avgScoreEvals,
          hasEvals ? `${perfOverview.avg_score} / 5` : "—",
          <Award size={20} className="text-primary-600" />,
          "bg-primary-50",
          hasEvals ? `${perfOverview.total_evals} ${t.analytics.evaluations}` : t.analytics.noEvaluation
        )}
        {renderKPICard(
          t.analytics.okrCompleted,
          okr ? okr.completed : "—",
          <TrendingUp size={20} className="text-emerald-600" />,
          "bg-emerald-50",
          okr ? t.analytics.onTotalObjectives.replace("{total}", String(okr.total)) : undefined
        )}
        {renderKPICard(
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-gray-900">{t.analytics.avgScoreByDept}</h3>
                      </div>
          {byDept.length > 0 ? (
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={byDept} layout="vertical">
        ...
        <div className="bg-white rounded-xl border p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-gray-900">{t.analytics.evalDistribution}</h3>
                      </div>
          {dist.some((d: any) => d.count > 0) ? (
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={dist}>
                ...
                <YAxis tick={{ fontSize: 11 }} />
                <Tooltip />
                <Bar dataKey="count" name={t.analytics.employees} radius={[4, 4, 0, 0]}>
      ...
      {hasOkr && (
        <div className="bg-white rounded-xl border p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-gray-900">{t.analytics.okrStatus}</h3>
                      </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: "On Track", value: okr.on_track, color: "text-green-600 bg-green-50", border: "border-green-200" },
              { label: t.analytics.completed, value: okr.completed, color: "text-primary-600 bg-primary-50", border: "border-primary-200" },
              { label: t.analytics.atRiskLabel, value: okr.at_risk, color: "text-amber-600 bg-amber-50", border: "border-amber-200" },
              { label: t.analytics.totalActive, value: okr.total, color: "text-gray-600 bg-gray-50", border: "border-gray-200" },
            ].map((item, i) => (
      ...
      <div className="bg-white rounded-xl border p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-gray-900">{t.analytics.perfByManager}</h3>
                  </div>
        {perfByManager.length > 0 ? (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b text-left text-gray-500">
                  <th className="pb-3 font-medium">{t.analytics.manager}</th>
                  <th className="pb-3 font-medium">{t.analytics.team}</th>
                  <th className="pb-3 font-medium text-center">{t.analytics.size}</th>
                  <th className="pb-3 font-medium text-center">{t.analytics.avgScore}</th>
                </tr>
              </thead>
              <tbody>
          ...
          <p className="text-center text-gray-400 text-sm py-8">{t.analytics.noManagerEval}</p>
        )}
      </div>
    </div>
    );
  ...
  const renderTalents = () => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mt-3">
        ...
        {renderKPICard(
          t.analytics.coveredPositions,
          successionData ? `${coveredPct}%` : "—",
          <Target size={20} className="text-emerald-600" />, "bg-emerald-50",
          successionData ? `${successionData.total_plans} ${t.analytics.plans}` : undefined
        )}
        {renderKPICard(
          t.analytics.atRiskNineBox,
          hasNinebox ? atRisk : "—",
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-gray-900">{t.analytics.nineBoxDistribution}</h3>
                      </div>
          {hasNinebox ? (
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
            ...
            <div className="flex flex-col items-center justify-center h-[300px] text-gray-400">
              ...
              <p className="text-sm font-medium">{t.analytics.noNineBoxPlacement}</p>
              <p className="text-xs mt-1">{t.analytics.evaluateInTalents}</p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-xl border p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-gray-900">{t.analytics.successionCoverage}</h3>
                      </div>
            ...
            {plans.length > 0 ? plans.map((s: any, i: number) => (
              <div key={i} className="flex items-center gap-4">
                ...
                <div className="text-center">
                  ...
                  <p className="text-[10px] text-gray-400">{t.analytics.successors}</p>
                </div>
                <div className="text-center">
                  <p className="text-sm font-medium">{s.pret}</p>
                  <p className="text-[10px] text-gray-400">{t.analytics.ready}</p>
                </div>
                <div className="w-24">
              ...
              <div className="flex flex-col items-center justify-center h-48 text-gray-400">
                ...
                <p className="text-sm font-medium">{t.analytics.noSuccessionPlan}</p>
                <p className="text-xs mt-1">{t.analytics.configureInTalents}</p>
              </div>
            )}
          </div>
        </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-3">
        {renderKPICard(
          ...
          "bg-primary-50",
          fo ? `${fo.completed} / ${fo.total_assignments} ${t.analytics.formations}` : undefined
        )}
        {renderKPICard(
          t.analytics.completedTrainings,
          fo?.completed ?? "—",
          <GraduationCap size={20} className="text-green-600" />,
          "bg-green-50",
          fo ? `${fo.in_progress} ${t.analytics.inProgress}` : undefined
        )}
        {renderKPICard(
          t.analytics.availableCourses,
          fo?.total_courses ?? "—",
          <Clock size={20} className="text-purple-600" />,
          "bg-purple-50",
          t.analytics.trainingCatalog
        )}
        {renderKPICard(
          t.analytics.totalAssigned,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-gray-900">{t.analytics.trainingByCategory}</h3>
                      </div>
          {hasCat ? (
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={formationByCategory}>
                ...
                <Tooltip />
                <Legend />
                <Bar dataKey="nb_assignments" name={t.analytics.assigned} fill="#93c5fd" radius={[4, 4, 0, 0]} />
                <Bar dataKey="nb_completed" name={t.analytics.completedLabel} fill="#066C6C" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          ) : (
            <div className="flex flex-col items-center justify-center h-[300px] text-gray-400">
              <GraduationCap size={40} className="mb-3 opacity-30" />
              <p className="text-sm font-medium">{t.analytics.noTrainingByCategory}</p>
              <p className="text-xs mt-1">{t.analytics.assignTrainings}</p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-xl border p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-gray-900">{t.analytics.monthlyEvolution}</h3>
                      </div>
          {hasEvol ? (
            <ResponsiveContainer width="100%" height={300}>
              <AreaChart data={formationEvolution}>
                ...
                <Tooltip />
                <Legend />
                <Area type="monotone" dataKey="assignees" name={t.analytics.assigned} stroke="#93c5fd" fill="#93c5fd20" strokeWidth={2} />
                <Area type="monotone" dataKey="completes" name={t.analytics.completedLabel} stroke="#066C6C" fill="#066C6C20" strokeWidth={2} />
              </AreaChart>
            </ResponsiveContainer>
          ) : (
            <div className="flex flex-col items-center justify-center h-[300px] text-gray-400">
              <TrendingUp size={40} className="mb-3 opacity-30" />
              <p className="text-sm font-medium">{t.analytics.noEvolution}</p>
              <p className="text-xs mt-1">{t.analytics.dataOverMonths}</p>
            </div>
          )}
        </div>
      </div>
      ...
      {fo && (
        <div className="bg-white rounded-xl border p-6">
          <h3 className="font-semibold text-gray-900 mb-4">{t.analytics.statusDistribution}</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: t.analytics.completedLabel, value: fo.completed, color: "bg-green-500", pct: fo.total_assignments > 0 ? Math.round(fo.completed / fo.total_assignments * 100) : 0 },
              { label: t.analytics.inProgressLabel, value: fo.in_progress, color: "bg-primary-500", pct: fo.total_assignments > 0 ? Math.round(fo.in_progress / fo.total_assignments * 100) : 0 },
              { label: t.analytics.availableCourses, value: fo.total_courses, color: "bg-purple-500", pct: 100 },
              { label: t.analytics.totalAssigned, value: fo.total_assignments, color: "bg-gray-400", pct: 100 },
            ].map((item, i) => (
  ...
  const renderEngagement = () => {
      ...
      return (
        <div className="space-y-6">
          <div className="flex flex-col items-center justify-center py-20 text-gray-400">
            ...
            <p className="text-lg font-semibold text-gray-600">{t.analytics.noFlashSurvey}</p>
            <p className="text-sm text-gray-400 mt-2 mb-6 text-center max-w-md">
              {t.analytics.flashSurveyDescription}
            </p>
            <button
    ...
    if (engLoading) {
      return (
        <div className="flex items-center justify-center py-20">
          ...
          <span className="ml-3 text-gray-500">{t.analytics.loadingEngagement}</span>
        </div>
      );
    }
    ...
    const getEngagementColor = (score: number) => {
      if (score < 4) return { bg: "bg-red-500", text: "text-red-600", label: t.analytics.critical };
      if (score < 6) return { bg: "bg-orange-500", text: "text-orange-600", label: t.analytics.toWatch };
      if (score < 8) return { bg: "bg-lime-500", text: "text-lime-600", label: t.analytics.good };
      return { bg: "bg-green-600", text: "text-green-700", label: t.analytics.excellent };
    };
    ...
    <div className="space-y-6">
      ...
      {rpsAlerts.length > 0 ? (
        ...
        <div className="flex items-center gap-3 p-4 bg-green-50 border border-green-200 rounded-xl">
          ...
          <p className="text-sm text-green-800 font-medium">{t.analytics.allIndicatorsNormal}</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-3">
        ...
        {renderKPICard(
          t.analytics.participationRate,
          `${participationRate}%`,
          <Users size={20} className="text-primary-600" />,
          "bg-primary-50",
          `${totalCompleted}/${totalInvited} ${t.analytics.responses}`
        )}
        {renderKPICard(
          t.analytics.flashSurveys,
          pulseSurveys.length,
          <Activity size={20} className="text-purple-600" />,
          "bg-purple-50",
          `${pulseSurveys.filter(s => s.score > 0).length} ${t.analytics.withResults}`
        )}
        {renderKPICard(
          t.analytics.rpsAlerts,
          rpsAlerts.length,
          <AlertTriangle size={20} className={rpsAlerts.length > 0 ? "text-red-600" : "text-green-600"} />,
          rpsAlerts.length > 0 ? "bg-red-50" : "bg-green-50",
          rpsAlerts.length > 0 ? t.analytics.scoreLessThan5 : t.analytics.noAlertLabel
        )}
      </div>
      ...
      {engagementIndex != null && engColor && (
        <div className="bg-white rounded-xl border p-6">
          <h3 className="font-semibold text-gray-900 mb-4">{t.analytics.globalEngagementIndex}</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-gray-900">{t.analytics.engagementTrend}</h3>
          </div>
          {trendData.length >= 2 ? (
            <ResponsiveContainer width="100%" height={300}>
            ...
            <div className="flex flex-col items-center justify-center h-[300px] text-gray-400">
              <TrendingUp size={40} className="mb-3 opacity-30" />
              <p className="text-sm font-medium">{t.analytics.notEnoughData}</p>
              <p className="text-xs mt-1">{t.analytics.atLeast2Surveys}</p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-xl border p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-gray-900">{t.analytics.participationBySurvey}</h3>
          </div>
          {participationData.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center h-[300px] text-gray-400">
              ...
              <p className="text-sm font-medium">{t.analytics.noParticipationData}</p>
            </div>
          )}
        </div>
      </div>
      ...
      {satisfactionFactors.length > 0 && (
        <div className="bg-white rounded-xl border p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-gray-900">{t.analytics.satisfactionFactors}</h3>
            <span className="text-xs text-gray-400">{t.analytics.avgScorePerQuestion}</span>
          </div>
          <div className="space-y-3">
  ...
  const renderRecrutement = () => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mt-3">
        {renderKPICard(
          t.analytics.openPositions,
          stats?.open_positions ?? "8",
          <Briefcase size={20} className="text-primary-600" />, "bg-primary-50"
        )}
        {renderKPICard(
          t.analytics.activeCandidates,
          stats?.total_candidates ?? "916",
          <FileText size={20} className="text-purple-600" />, "bg-purple-50",
          stats ? `${stats.in_interview} ${t.analytics.inInterview}` : undefined
        )}
        {renderKPICard(
          t.analytics.hiresYear,
          stats?.hires_this_year ?? "50",
          <UserPlus size={20} className="text-green-600" />, "bg-green-50",
          stats ? `${stats.hires_this_month} ${t.analytics.thisMonth}` : undefined
        )}
        {renderKPICard(
          t.analytics.avgHiringDelay,
          stats?.avg_time_to_hire != null ? `${stats.avg_time_to_hire} ${t.analytics.days}` : `32 ${t.analytics.days}`,
          <Clock size={20} className="text-amber-600" />, "bg-amber-50"
        )}
        {renderKPICard(
          t.analytics.inInterviewLabel,
          stats?.in_interview ?? "—",
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {pipeline.length > 0 ? (
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={pipeline} layout="vertical">
                ...
                <Tooltip />
                <Bar dataKey="count" name={t.analytics.candidates} radius={[0, 4, 4, 0]}>
        ...
        {/* Tendance mensuelle */}
        <div className="bg-white rounded-xl border p-6">
          <h3 className="font-semibold text-gray-900 mb-4">{t.analytics.monthlyTrend}</h3>
          <ResponsiveContainer width="100%" height={300}>
      ...
      <div className="bg-white rounded-xl border p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-gray-900">{t.analytics.recruitmentSources}</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b text-left text-gray-500">
                <th className="pb-3 font-medium">{t.analytics.source}</th>
                <th className="pb-3 font-medium text-center">{t.analytics.candidatures}</th>
                <th className="pb-3 font-medium text-center">{t.analytics.percentTotal}</th>
              </tr>
            </thead>
            <tbody>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mt-3">
        ...
        {renderKPICard(
          ...
          "bg-emerald-50",
          salaireOverview ? `${t.analytics.avgRaise}: +${salaireOverview.pct_augmentation_moy}%` : undefined
        )}
        {renderKPICard(
          t.analytics.paidEmployees,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <AreaChart data={salaireEvolution}>
              ...
              <Area
                ...
                name={t.analytics.avgSalaryLabel}
                stroke="#10b981"
                fill="#10b98120"
                strokeWidth={2}
              />
        ...
        <div className="bg-white rounded-xl border p-6">
          <h3 className="font-semibold text-gray-900 mb-4">{t.analytics.deptDistributionSalary}</h3>
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie
                data={salaireByDept}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border p-6">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={salaireDistribution}>
              ...
              />
              <Bar dataKey="nb_employes" name={t.analytics.effectif} radius={[4, 4, 0, 0]}>
        ...
        <div className="bg-white rounded-xl border p-6">
          <h3 className="font-semibold text-gray-900 mb-4">{t.analytics.salaryByContract}</h3>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={salaireByContract} layout="vertical">
              ...
              <Tooltip formatter={(v: number) => `${formatXOF(v)} XOF`} />
              <Bar dataKey="masse_salariale" name={t.analytics.salaryMass} radius={[0, 4, 4, 0]}>
      ...
      <div className="bg-white rounded-xl border p-6">
        <h3 className="font-semibold text-gray-900 mb-4">{t.analytics.deptDetail}</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b text-left text-gray-500">
                <th className="pb-3 font-medium">{t.common.department}</th>
                <th className="pb-3 font-medium text-center">{t.analytics.effectif}</th>
                <th className="pb-3 font-medium text-right">{t.analytics.salaryPerMonth}</th>
                <th className="pb-3 font-medium text-right">{t.analytics.avgSalaryLabel}</th>
                <th className="pb-3 font-medium text-center">{t.analytics.percentOfTotal}</th>
              </tr>
            </thead>
            <tbody>
          ...
          {salaireByDept.length === 0 && (
            <p className="text-center text-gray-400 py-8 text-sm">{t.analytics.noSalaryData}</p>
          )}
        </div>
      </div>
    </div>
  ...
  const renderImpactFormation = () => {
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mt-3">
          {renderKPICard(
            t.analytics.activePlans,
            nbPlansActifs || "—",
            <BookOpen size={20} className="text-primary-600" />,
            "bg-primary-50",
            `${allPlans.length} ${t.analytics.totalPlans}`
          )}
          {renderKPICard(
            t.analytics.realizationRate,
            allPlans.length > 0 ? `${tauxRealisationMoyen}%` : "—",
            <Target size={20} className="text-green-600" />,
            "bg-green-50",
            t.analytics.allTrainingsAvg
          )}
          {renderKPICard(
            t.analytics.trainedEmployees,
            nbEmployesFormes || "—",
          ...
          {renderKPICard(
            t.analytics.trainingBudget,
            budgetTotal > 0 ? `${formatXOF(budgetConsomme)} / ${formatXOF(budgetTotal)}` : "—",
            <Banknote size={20} className="text-amber-600" />,
            "bg-amber-50",
            budgetTotal > 0 ? `${Math.round((budgetConsomme / budgetTotal) * 100)}% ${t.analytics.consumed}` : undefined
          )}
          {renderKPICard(
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-xl border p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-gray-900">{t.analytics.trainedByDept}</h3>
                          </div>
            {deptChartData.length > 0 ? (
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={deptChartData}>
                  ...
                  <XAxis dataKey="name" tick={{ fontSize: 11 }} />
                  <YAxis tick={{ fontSize: 11 }} />
                  <Tooltip formatter={(v: number) => [v, t.analytics.trainedEmployeesLabel]} />
                  <Bar dataKey="employes" name={t.analytics.trainedEmployeesLabel} radius={[4, 4, 0, 0]}>
              ...
              <div className="flex flex-col items-center justify-center h-[300px] text-gray-400">
                ...
                <p className="text-sm font-medium">{t.analytics.noDeptData}</p>
                <p className="text-xs mt-1">{t.analytics.createTrainingPlans}</p>
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-xl border p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-gray-900">{t.analytics.okrImpact}</h3>
                          </div>
            {plansWithOkr.length > 0 ? (
                  ...
                  return (
                    <div key={i} className="border rounded-lg p-4">
                      ...
                      <div className="flex items-center gap-4 text-xs text-gray-500">
                        <span>{t.analytics.before} : {okr.progress_before ?? 0}%</span>
                        <span>{t.analytics.current} : {okr.progress_current ?? 0}%</span>
                      </div>
              ...
              <div className="flex flex-col items-center justify-center h-[300px] text-gray-400">
                ...
                <p className="text-sm font-medium">{t.analytics.noOkrLinked}</p>
                <p className="text-xs mt-1">{t.analytics.linkOkrToPlans}</p>
              </div>
            )}
          </div>
        </div>
        ...
        {hasSubsidiaries && (
          <div className="bg-white rounded-xl border p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-semibold text-gray-900">{t.analytics.subsidiaryComparison}</h3>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b text-left text-gray-500">
                    <th className="pb-3 font-medium">{t.analytics.subsidiary}</th>
                    <th className="pb-3 font-medium text-center">{t.analytics.activePlans}</th>
                    <th className="pb-3 font-medium text-center">{t.analytics.trainedEmployeesCol}</th>
                    <th className="pb-3 font-medium text-center">{t.analytics.realizationRateCol}</th>
                    <th className="pb-3 font-medium text-right">{t.analytics.budgetConsumed}</th>
                  </tr>
                </thead>
                <tbody>
              ...
              {subsidiariesList.length === 0 && (
                <p className="text-center text-gray-400 py-8 text-sm">{t.analytics.noSubsidiaryAvailable}</p>
              )}
            </div>
          </div>
        )}
    ...
    <div className="p-6 max-w-[1400px] mx-auto">
      ...
      {loading && (
        <div className="flex items-center justify-center py-12">
          ...
          <span className="ml-3 text-gray-500">{t.analytics.loadingData}</span>
        </div>
      )}
