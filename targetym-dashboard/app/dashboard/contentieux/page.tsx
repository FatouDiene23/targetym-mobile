...
function getStatusInfo(key: string) {
  return STATUSES[key] || { label: key, color: 'bg-gray-100 text-gray-600' };
}
...
export default function ContentieuxPage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      toast.success('Dossier creee avec succes');
  ...
  const handleChangeStage = async () => {
    if (!stageForm.stage || !stageForm.notes.trim()) {
      toast.error('L\'etape et les notes sont obligatoires');
      return;
    }
  ...
  const handleCreateAudience = async () => {
    ...
    try {
      ...
      toast.success('Audience planifiee');
  ...
  const handleDeleteAudience = async (audienceId: number) => {
    ...
    try {
      ...
      toast.success('Audience supprimee');
      loadDetail(selectedDispute.id);
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : 'Erreur';
      toast.error(msg);
      ...
      <div className="p-6 w-full">
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <div className="flex items-center gap-4 text-sm text-gray-500">
                ...
                <span className="flex items-center gap-1"><Calendar className="w-3.5 h-3.5" /> Ouvert le {formatDate(d.opened_date)}</span>
            ...
            <div className="flex gap-2">
              {!isClosed && (
                ...
                >
                  <ArrowRight className="w-4 h-4" /> Changer d&apos;etape
                </button>
              )}
              <button
          ...
          <div className="border-b">
            <nav className="flex">
              {[
                ...
                { key: 'audiences' as const, label: 'Audiences', icon: Gavel },
                { key: 'documents' as const, label: 'Documents', icon: FileText },
                { key: 'infos' as const, label: 'Infos', icon: AlertCircle },
              ].map(tab => (
                <button
                  key={tab.key}
                          ...
                          return (
                            <div key={h.id} className="relative pl-12 pb-6">
                              ...
                              <div className={`${si.bg} rounded-lg p-4 border ${si.color.split(' ').find(c => c.startsWith('border-')) || ''}`}>
                                ...
                                {h.created_by_name && <p className="text-xs text-gray-400 mt-1">Par {h.created_by_name}</p>}
                              </div>
                            </div>
                          );
                        })}
                  ...
                  <div>
                    {!isClosed && (
                      <div className="flex justify-end mb-4">
                        ...
                        >
                          <Plus className="w-4 h-4" /> Planifier une audience
                        </button>
                      </div>
                    )}
                    {(d.audiences || []).length === 0 ? (
                      <p className="text-sm text-gray-400 text-center py-8">Aucune audience planifiee</p>
                    ) : (
                      <div className="space-y-3">
                        {(d.audiences || []).map(a => (
                          <div key={a.id} className="bg-gray-50 rounded-lg p-4 flex items-start justify-between">
                            <div>
                              <div className="flex items-center gap-2 mb-1">
                                <span className="text-sm font-medium text-gray-900">{AUDIENCE_TYPES[a.audience_type] || a.audience_type}</span>
                                <span className="text-xs text-gray-400">{formatDateTime(a.audience_date)}</span>
                              </div>
                              {a.location && <p className="text-xs text-gray-500">Lieu : {a.location}</p>}
                              {a.notes && <p className="text-sm text-gray-600 mt-1">{a.notes}</p>}
                              {a.result && <p className="text-sm text-gray-700 mt-1 font-medium">Resultat : {a.result}</p>}
                              {a.next_audience_date && <p className="text-xs text-primary-600 mt-1">Prochaine audience : {formatDate(a.next_audience_date)}</p>}
                            </div>
                            {!isClosed && (
                ...
                {detailTab === 'infos' && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 lg:gap-6">
                    <div className="space-y-4">
                      ...
                      <div>
                        <label className="text-xs font-medium text-gray-500 uppercase">Employe</label>
                        <p className="text-sm text-gray-900">{d.employee_name || `#${d.employee_id}`}</p>
                      </div>
                      <div>
                        <label className="text-xs font-medium text-gray-500 uppercase">Objet du litige</label>
                    ...
                    <div className="space-y-4">
                      <div>
                        <label className="text-xs font-medium text-gray-500 uppercase">Juriste assigne</label>
                        <p className="text-sm text-gray-900">{d.assigned_to_name || 'Non assigne'}</p>
                      </div>
                      <div>
                      ...
                      {d.closed_date && (
                        <div>
                          <label className="text-xs font-medium text-gray-500 uppercase">Date de cloture</label>
                          <p className="text-sm text-gray-900">{formatDate(d.closed_date)}</p>
                        </div>
                      )}
                      <div>
                        <label className="text-xs font-medium text-gray-500 uppercase">Cree par</label>
                        <p className="text-sm text-gray-900">{d.created_by_name || '-'}</p>
                      </div>
                      <div>
                        <label className="text-xs font-medium text-gray-500 uppercase">Cree le</label>
        ...
        {showStageModal && (
          <ModalOverlay onClose={() => setShowStageModal(false)}>
            <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
              <ArrowRight className="w-5 h-5 text-primary-500" /> Changer d&apos;etape
            </h3>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Nouvelle etape *</label>
                <select
              ...
              {stageForm.stage === 'conciliation' && (
                <div className="bg-purple-50 border border-purple-200 rounded-lg p-3">
                  <p className="text-sm text-purple-700">La conciliation peut aboutir a un accord amiable (selectionnez &quot;Accord amiable&quot;) ou echouer (selectionnez &quot;Contentieux&quot;).</p>
                </div>
              )}
              {stageForm.stage === 'accord_amiable' && (
                <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                  <p className="text-sm text-green-700">Le dossier sera clos avec le statut &quot;Clos (accord)&quot;.</p>
                </div>
              )}
              {stageForm.stage === 'cloture' && (
                <div className="bg-gray-50 border border-gray-200 rounded-lg p-3">
                  <p className="text-sm text-gray-700">Le dossier sera clos avec le statut &quot;Clos (jugement)&quot;.</p>
                </div>
              )}
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder="Decrivez les raisons du changement d'etape..."
            ...
            <div className="mt-6 flex justify-end gap-3">
              <button onClick={() => setShowStageModal(false)} className="px-4 py-2 border border-gray-300 rounded-lg text-sm text-gray-700 hover:bg-gray-50">Annuler</button>
              <button
              ...
              >
                {isSaving && <Loader2 className="w-4 h-4 animate-spin" />} Confirmer
              </button>
            </div>
          </ModalOverlay>
        )}
        ...
        {showAudienceModal && (
          <ModalOverlay onClose={() => setShowAudienceModal(false)}>
            <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
              <Gavel className="w-5 h-5 text-primary-500" /> Planifier une audience
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 outline-none"
                >
                  {Object.entries(AUDIENCE_TYPES).map(([k, v]) => (
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Prochaine audience</label>
                <input
                  type="date"
            ...
            <div className="mt-6 flex justify-end gap-3">
              <button onClick={() => setShowAudienceModal(false)} className="px-4 py-2 border border-gray-300 rounded-lg text-sm text-gray-700 hover:bg-gray-50">Annuler</button>
              <button
                onClick={handleCreateAudience}
                disabled={isSaving || !audienceForm.audience_date}
                className="flex items-center gap-2 px-4 py-2 bg-primary-500 text-white text-sm font-medium rounded-lg hover:bg-primary-600 disabled:opacity-50"
              >
                {isSaving && <Loader2 className="w-4 h-4 animate-spin" />} Planifier
              </button>
            </div>
          </ModalOverlay>
        )}
        ...
        {showEditModal && (
          <ModalOverlay onClose={() => setShowEditModal(false)}>
            <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
              <Edit3 className="w-5 h-5 text-primary-500" /> Modifier le dossier
            </h3>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Objet du litige</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Juriste assigne</label>
                <select
            ...
            <div className="mt-6 flex justify-end gap-3">
              <button onClick={() => setShowEditModal(false)} className="px-4 py-2 border border-gray-300 rounded-lg text-sm text-gray-700 hover:bg-gray-50">Annuler</button>
              <button
    ...
    <div className="p-6 w-full">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full">
          <div className="relative flex-1 min-w-[250px]">
            ...
            <input
              ...
              placeholder="Rechercher par reference, employe, objet..."
      ...
      <div className="bg-white rounded-xl shadow-sm border overflow-hidden w-full">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-gray-400">
            ...
            <p className="text-lg font-medium text-gray-500">Aucun dossier contentieux</p>
            <p className="text-sm text-gray-400 mt-1">Les dossiers de litiges apparaitront ici</p>
          </div>
        ) : (
          <>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    ...
                    <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Employe</th>
                    <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Objet</th>
                    <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Etape</th>
                    <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Statut</th>
                    <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Ouverture</th>
                    <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Juriste</th>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between px-4 py-3 border-t">
                <p className="text-sm text-gray-500">{total} dossier{total > 1 ? 's' : ''}</p>
                <div className="flex items-center gap-2">
                  <button
      ...
      {showCreateModal && (
        <ModalOverlay onClose={() => setShowCreateModal(false)}>
          <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
            <Scale className="w-5 h-5 text-primary-500" /> Nouveau dossier contentieux
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Employe *</label>
              <select
            ...
            <div className="md:col-span-2">
              ...
              <input
                ...
                placeholder="Ex: Licenciement conteste"
                className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 outline-none"
              />
            </div>
            <div className="md:col-span-2">
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 outline-none"
              >
                <option value="">Selectionner un juriste</option>
                {juristes.map(j => (
                  <option key={j.id} value={j.id}>{j.first_name} {j.last_name}</option>
                ))}
              </select>
          ...
          <div className="mt-6 flex justify-end gap-3">
            <button onClick={() => setShowCreateModal(false)} className="px-4 py-2 border border-gray-300 rounded-lg text-sm text-gray-700 hover:bg-gray-50">Annuler</button>
            <button
              onClick={handleCreate}
              disabled={isSaving || !createForm.employee_id || !createForm.title.trim() || !createForm.reference_number.trim()}
              className="flex items-center gap-2 px-4 py-2 bg-primary-500 text-white text-sm font-medium rounded-lg hover:bg-primary-600 disabled:opacity-50"
            >
              {isSaving && <Loader2 className="w-4 h-4 animate-spin" />} Creer le dossier
            </button>
          </div>
        </ModalOverlay>
      )}
