...
const DEPARTURE_TYPES: Record<string, { label: string; color: string }> = {
  ...
  end_of_contract: { label: 'Fin de contrat', color: 'bg-gray-100 text-gray-800' },
  transfer: { label: 'Transfert', color: 'bg-teal-100 text-teal-800' },
};

const STATUS_CONFIG: Record<string, { label: string; color: string; icon: typeof Clock }> = {
  draft: { label: 'Brouillon', color: 'bg-gray-100 text-gray-700', icon: Edit2 },
  pending_manager: { label: 'Attente Manager', color: 'bg-yellow-100 text-yellow-800', icon: Clock },
...
const CHECKLIST_CATEGORIES: Record<string, { label: string; color: string }> = {
  ...
  hr: { label: 'RH', color: 'text-green-600' },
...
async function fetchGroupSubsidiaries(): Promise<Array<{id: number, name: string}>> {
  try {
    ...
    // Pour un utilisateur filiale, récupérer les filiales sœurs via le groupe parent
...
export default function DeparturesPage() {
  ...
  const loadDepartures = useCallback(async () => {
    ...
    try {
      ...
      toast.error('Erreur de chargement des départs');
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    if (activeTab === 'statistiques') {
      fetchStats().then(setStats).catch(() => toast.error('Erreur chargement stats'));
    }
  }, [activeTab]);
  ...
  const handleCreate = async () => {
    ...
    if (formType === 'transfer' && !formTargetTenantId) {
      toast.error('Veuillez sélectionner une filiale de destination');
      return;
    }
    ...
    try {
      ...
      // Si transfert inter-filiales, exécuter immédiatement
      if (formType === 'transfer' && formTargetTenantId) {
        const transferRes = await fetch(`${API_URL}/api/departures/${created.id}/execute-transfer`, {
          method: 'POST', headers: getAuthHeaders(),
          body: JSON.stringify({ target_tenant_id: formTargetTenantId, transfer_date: formDate }),
        });
        if (!transferRes.ok) {
        ...
        toast.success(transferData.message || `${selectedEmployee.first_name} ${selectedEmployee.last_name} a été transféré(e) avec succès`);
      } else {
  ...
  const handleValidate = async (approved: boolean) => {
    ...
    try {
      ...
      toast.success(approved ? 'Départ validé' : 'Départ refusé');
  ...
  const toggleChecklistItem = async (itemId: number) => {
    ...
    try {
      ...
      toast.error('Erreur mise à jour checklist');
    }
  };
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      toast.success('Départ clôturé, employé archivé');
      setSelectedDep(null);
      loadDepartures();
    } catch (e: unknown) {
      toast.error(e instanceof Error ? e.message : 'Erreur clôture');
    }
  };
  ...
  const handleGenerateCertificate = async () => {
    ...
    try {
      ...
      toast.error('Erreur génération certificat');
    }
  };
  ...
  const renderTypeBadge = (type: string) => {
    const cfg = DEPARTURE_TYPES[type] || { label: type, color: 'bg-gray-100 text-gray-700' };
    return <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${cfg.color}`}>{cfg.label}</span>;
  };

  // Render tabs
  const tabs = [
    { id: 'en_cours' as const, label: 'Départs en cours', icon: Clock },
    { id: 'historique' as const, label: 'Historique', icon: FileText },
    { id: 'statistiques' as const, label: 'Statistiques', icon: TrendingDown },
  ];

  return (
    <>
      <Header title="Gestion des Départs" subtitle="Offboarding et suivi des départs" />
              ...
              <div className="bg-white rounded-xl border overflow-hidden">
                {loading ? (
                  ...
                  <div className="text-center py-20 text-gray-500">
                    ...
                    <p className="font-medium">Aucun départ trouvé</p>
                  </div>
                ) : (
                  <table className="w-full">
                    <thead className="bg-gray-50 border-b">
                      <tr>
                        <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Collaborateur</th>
                        <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Type</th>
                        {!selectedDep && <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Date effective</th>}
                        {!selectedDep && <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Checklist</th>}
                            ...
                            <div className="flex items-center justify-end gap-1" onClick={e => e.stopPropagation()}>
                              {dep.status === 'pending_manager' && (
                                <button
                                  ...
                                  className="p-1.5 text-yellow-600 hover:bg-yellow-50 rounded-lg" title="Valider (Manager)"
                                ><CheckCircle className="w-4 h-4" /></button>
                              )}
                              {dep.status === 'pending_rh' && (
                                <button
                                  onClick={() => setShowValidation({ id: dep.id, type: 'rh' })}
                                  className="p-1.5 text-orange-600 hover:bg-orange-50 rounded-lg" title="Valider (RH)"
                                ><CheckCircle className="w-4 h-4" /></button>
                              )}
                              {dep.status === 'pending_direction' && (
                                <button
                                  onClick={() => setShowValidation({ id: dep.id, type: 'direction' })}
                                  className="p-1.5 text-red-600 hover:bg-red-50 rounded-lg" title="Valider (Direction)"
                                ><CheckCircle className="w-4 h-4" /></button>
                              )}
                  ...
                  <>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            {selectedDep.employee_hire_date && (
                              <p className="text-xs text-gray-400">Embauché le {formatDate(selectedDep.employee_hire_date)}</p>
                            )}
                          </div>
                        </div>
                    ...
                    <div className="flex border-b">
                      {([
                        ...
                        { id: 'entretien' as const, label: 'Entretien', icon: MessageSquare },
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          <h4 className="text-sm font-semibold text-gray-900">Processus de validation</h4>
          <div className="space-y-2">
            {selectedDep.initiated_by_role === 'employee' && (
              <>
                <ValidationStep label="Validation Manager" done={!!selectedDep.manager_validated_at}
                  date={selectedDep.manager_validated_at} comment={selectedDep.manager_comment}
                  active={selectedDep.status === 'pending_manager'} />
                <ValidationStep label="Validation RH" done={!!selectedDep.rh_validated_at}
            ...
            {selectedDep.departure_type === 'termination' && (
              <ValidationStep label="Validation Direction" done={!!selectedDep.direction_validated_at}
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <InfoCard label="Fin de préavis" value={formatDate(selectedDep.notice_end_date)} />
          <InfoCard label="Date effective" value={formatDate(selectedDep.effective_date)} />
          <InfoCard label="Dernier jour" value={formatDate(selectedDep.last_working_day)} />
        </div>
        ...
        {selectedDep.reason && (
          <div>
            <h4 className="text-sm font-semibold text-gray-900 mb-1">Motif</h4>
            <p className="text-sm text-gray-600">{selectedDep.reason}</p>
          </div>
        )}
        {selectedDep.legal_reason && (
          <div className="bg-red-50 rounded-lg p-3">
            <h4 className="text-sm font-semibold text-red-900 mb-1">Motif légal</h4>
            <p className="text-sm text-red-700">{selectedDep.legal_reason}</p>
          </div>
        )}
        {selectedDep.detailed_reason && (
          <div>
            <h4 className="text-sm font-semibold text-gray-900 mb-1">Détails</h4>
        ...
        <div className="flex flex-wrap gap-2 pt-2">
          ...
          {['validated', 'in_progress'].includes(selectedDep.status) && (
      ...
      <div className="space-y-4">
        ...
        {totalItems === 0 ? (
          <div className="text-center py-8 text-gray-500">
            ...
            <p className="text-sm">La checklist sera générée à la validation du départ</p>
          </div>
        ) : (
          Object.entries(grouped).map(([cat, catItems]) => (
            <div key={cat}>
              <h4 className={`text-xs font-semibold uppercase tracking-wider mb-2 ${CHECKLIST_CATEGORIES[cat]?.color || 'text-gray-600'}`}>
                {CHECKLIST_CATEGORIES[cat]?.label || cat}
              </h4>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-400 mt-0.5">
                        Assigné : {item.assigned_to.toUpperCase()}
                        {item.is_required && <span className="ml-2 text-red-400">Requis</span>}
                        {item.completed_at && <span className="ml-2">- Fait le {formatDate(item.completed_at)}</span>}
                      </p>
                    </div>
                  </label>
                ))}
            ...
            <div className="bg-gray-50 rounded-lg p-4 space-y-3">
              ...
              <div>
                ...
                <input type="text" value={interviewLocation} onChange={e => setInterviewLocation(e.target.value)}
                  placeholder="Bureau RH, salle de réunion..."
                  className="w-full border rounded-lg px-3 py-1.5 text-sm mt-1" />
              </div>
    ...
    return (
      <div className="space-y-4">
        ...
        {interview.status === 'planned' || interview.status === 'completed' ? (
          <div className="space-y-4">
            <h4 className="text-sm font-semibold text-gray-900">Questionnaire de sortie</h4>

            {/* Ratings */}
            <div className="space-y-3">
              {[
                { key: 'departure_reason_rating', label: 'Satisfaction globale' },
                { key: 'management_rating', label: 'Management' },
                { key: 'work_environment_rating', label: 'Environnement de travail' },
                { key: 'career_growth_rating', label: 'Évolution de carrière' },
                { key: 'compensation_rating', label: 'Rémunération' },
              ].map(({ key, label }) => (
                <div key={key}>
              ...
              <div>
                ...
                <div className="flex gap-2 mt-1">
                  <button onClick={() => setInterviewForm(prev => ({ ...prev, would_recommend: true }))}
                    ...
                    }`}><ThumbsUp className="w-3.5 h-3.5" /> Oui</button>
                  <button onClick={() => setInterviewForm(prev => ({ ...prev, would_recommend: false }))}
                    ...
                    }`}><ThumbsDown className="w-3.5 h-3.5" /> Non</button>
                </div>
              </div>
              <div>
                <label className="text-xs text-gray-500">Reviendrait ?</label>
                <div className="flex gap-2 mt-1">
            ...
            {/* Text fields */}
            {[
              { key: 'positive_aspects', label: 'Aspects positifs' },
              { key: 'improvement_areas', label: 'Axes d\'amélioration' },
              { key: 'suggestions', label: 'Suggestions' },
              { key: 'additional_notes', label: 'Notes complémentaires' },
            ].map(({ key, label }) => (
              <div key={key}>
                <label className="text-xs text-gray-500">{label}</label>
                <textarea
    ...
    return (
      <div className="space-y-4">
        <div className="bg-white border rounded-lg p-4 space-y-3">
          <h4 className="text-sm font-semibold text-gray-900">Générer les documents</h4>
          ...
          <button onClick={handleGenerateSettlement}
            ...
            <div>
              <p className="text-sm font-medium text-gray-900">Solde de tout compte</p>
              <p className="text-xs text-gray-500">Détail des indemnités : congés ({selectedDep.leave_balance_days}j), compensation</p>
            </div>
            <Download className="w-4 h-4 text-gray-400 ml-auto" />
          </button>
        </div>
        ...
        <div className="bg-amber-50 rounded-lg p-3 flex items-start gap-2">
          ...
          <p className="text-xs text-amber-700">
            Les documents seront générés avec les informations actuelles du collaborateur.
            Assurez-vous que toutes les données sont à jour avant la génération.
          </p>
        </div>
      </div>
    );
  ...
  function renderStatsTab() {
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-white rounded-xl border p-5">
            <p className="text-sm text-gray-500">Taux de turnover</p>
            <p className="text-3xl font-bold text-red-600">{stats.turnover_rate}%</p>
            <p className="text-xs text-gray-400 mt-1">{stats.year}</p>
          </div>
          <div className="bg-white rounded-xl border p-5">
            <p className="text-sm text-gray-500">Total départs</p>
            <p className="text-3xl font-bold text-gray-900">{stats.total_departures}</p>
            <p className="text-xs text-gray-400 mt-1">sur {stats.total_active_employees} actifs</p>
          </div>
          <div className="bg-white rounded-xl border p-5">
            <p className="text-sm text-gray-500">Ancienneté moy. départs</p>
            <p className="text-3xl font-bold text-gray-900">{stats.avg_seniority_years} ans</p>
          </div>
          <div className="bg-white rounded-xl border p-5">
            <p className="text-sm text-gray-500">Raisons d&apos;entretien</p>
            <p className="text-3xl font-bold text-gray-900">{stats.departure_reasons.length}</p>
            <p className="text-xs text-gray-400 mt-1">catégories distinctes</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-xl border p-5">
            ...
            <div className="space-y-3">
              {stats.by_type.map(t => {
                const cfg = DEPARTURE_TYPES[t.type] || { label: t.type, color: 'bg-gray-100 text-gray-700' };
                const pct = Math.round((t.count / Math.max(stats.total_departures, 1)) * 100);
                return (
          ...
          <div className="bg-white rounded-xl border p-5 lg:col-span-2">
            <h3 className="text-sm font-semibold text-gray-900 mb-4">Raisons principales de départ (entretiens)</h3>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                {stats.departure_reasons.map(r => (
                  <div key={r.reason} className="bg-gray-50 rounded-lg p-3 text-center">
                    ...
                    <p className="text-xs text-gray-500 mt-1">{DEPARTURE_REASONS[r.reason] || r.reason}</p>
                  </div>
                ))}
              </div>
            ) : (
              <p className="text-sm text-gray-400 text-center py-4">Aucun entretien de départ complété</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
          <div className="flex items-center justify-between p-5 border-b">
            <h2 className="text-lg font-semibold">Nouveau départ</h2>
          ...
          <div className="p-5 space-y-4">
            ...
            {createStep === 1 && (
              <div className="space-y-3">
                <h3 className="font-medium text-gray-900">1. Sélectionner le collaborateur</h3>
            ...
            {createStep === 2 && (
              <div className="space-y-3">
                ...
                {formType === 'termination' && (
                  <div>
                    <label className="text-sm text-gray-600">Motif légal <span className="text-red-500">*</span></label>
                    <textarea value={formLegalReason} onChange={e => setFormLegalReason(e.target.value)}
                      placeholder="Motif légal obligatoire pour un licenciement..."
                      rows={2} className="w-full border rounded-lg px-3 py-2 text-sm mt-1 resize-none" />
                  </div>
                )}
                {formType === 'transfer' && (
                  <div>
                    <label className="text-sm text-gray-600">Filiale de destination <span className="text-red-500">*</span></label>
                ...
                <div>
                  ...
                  <input type="text" value={formReason} onChange={e => setFormReason(e.target.value)}
                    placeholder="Motif du départ" className="w-full border rounded-lg px-3 py-2 text-sm mt-1" />
                </div>
                <div>
                  <label className="text-sm text-gray-600">Détails complémentaires</label>
                  <textarea value={formDetailedReason} onChange={e => setFormDetailedReason(e.target.value)}
                    rows={2} className="w-full border rounded-lg px-3 py-2 text-sm mt-1 resize-none" />
                </div>
                <div>
                  <label className="text-sm text-gray-600">Date de départ souhaitée <span className="text-red-500">*</span></label>
            ...
            {createStep === 3 && (
              <div className="space-y-3">
                <h3 className="font-medium text-gray-900">3. Récapitulatif</h3>
                <div className="bg-gray-50 rounded-lg p-4 space-y-2">
                  ...
                  {formType === 'transfer' && formTargetTenantId && (
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-500">Filiale de destination</span>
                ...
                <div className="bg-primary-50 rounded-lg p-3">
                  <p className="text-xs text-primary-700">
                    {formType === 'transfer'
                      ? `Cet employé sera immédiatement transféré vers ${subsidiaries.find(s => s.id === formTargetTenantId)?.name ?? 'la filiale sélectionnée'}. Son profil et ses soldes de congés seront copiés. Il ne sera plus modifiable depuis cette filiale.`
                      : formType === 'termination'
                      ? 'Ce licenciement sera soumis à la validation de la Direction.'
                      : 'Le préavis et le solde de congés seront calculés automatiquement. Le départ sera directement validé.'}
                  </p>
                </div>
              </div>
            )}
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md mx-4">
          <div className="p-5 border-b">
            <h2 className="text-lg font-semibold">Validation {typeLabel}</h2>
          </div>
          <div className="p-5 space-y-3">
            <p className="text-sm text-gray-600">Voulez-vous approuver ou refuser ce départ ?</p>
            <div>
              <label className="text-sm text-gray-500">Commentaire (optionnel)</label>
              <textarea
          ...
          <div className="flex items-center justify-end gap-2 p-5 border-t">
            <button onClick={() => setShowValidation(null)}
              className="px-4 py-2 text-sm text-gray-600 hover:bg-gray-50 rounded-lg">Annuler</button>
            ...
            <button onClick={() => handleValidate(true)} disabled={validationLoading}
              ...
              {validationLoading ? 'Validation...' : 'Approuver'}
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <div className={`flex items-start gap-3 p-2.5 rounded-lg ${active ? 'bg-yellow-50 border border-yellow-200' : done ? 'bg-green-50' : 'bg-gray-50'}`}>
      ...
      <div className="flex-1">
        ...
        {done && date && <p className="text-xs text-green-600">Validé le {formatDate(date)}</p>}
        {done && comment && <p className="text-xs text-green-700 mt-0.5">{comment}</p>}
        {active && <p className="text-xs text-yellow-600">En attente...</p>}
      </div>
    </div>
  );
}
