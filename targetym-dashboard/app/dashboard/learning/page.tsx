      ...
      <div className="mb-6">
        <div data-tour="learning-stats" className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          <div className="bg-white rounded-xl px-4 py-3 shadow-sm border border-gray-100">
            ...
            <p className="text-xs text-gray-500 mt-0.5">{t.training.availableTrainings}</p>
          </div>
          <div className="bg-white rounded-xl px-4 py-3 shadow-sm border border-gray-100">
            <p className="text-2xl font-bold text-green-600">{stats?.completion_rate ?? 0}%</p>
            <p className="text-xs text-gray-500 mt-0.5">{t.training.completionRate}</p>
          </div>
          <div className="bg-white rounded-xl px-4 py-3 shadow-sm border border-gray-100">
            <p className="text-2xl font-bold text-primary-600">{stats?.completed_this_month ?? 0}</p>
            <p className="text-xs text-gray-500 mt-0.5">{t.training.completedThisMonth}</p>
          </div>
          <div className="bg-white rounded-xl px-4 py-3 shadow-sm border border-gray-100">
            <p className="text-2xl font-bold text-indigo-600">{totalCatalogHours}h</p>
            <p className="text-xs text-gray-500 mt-0.5">{t.training.contentHours}</p>
          </div>
          <div className="bg-white rounded-xl px-4 py-3 shadow-sm border border-gray-100">
            <p className="text-2xl font-bold text-purple-600">{stats?.total_certifications ?? 0}</p>
            <p className="text-xs text-gray-500 mt-0.5">{t.training.certifications}</p>
          </div>
          <div className={`rounded-xl px-4 py-3 shadow-sm border ${(stats?.pending_validation ?? 0) > 0 ? 'bg-amber-50 border-amber-200' : 'bg-white border-gray-100'}`}>
            ...
            <p className={`text-xs mt-0.5 ${(stats?.pending_validation ?? 0) > 0 ? 'text-amber-600' : 'text-gray-500'}`}>{t.training.pendingValidation}</p>
          </div>
        </div>
      </div>
      <div data-tour="learning-filters" className="flex flex-wrap gap-4 mb-6">
        ...
        {hasPermission(userRole, 'create_course') && (
          <button onClick={() => setShowCreateCourse(true)} className="flex items-center px-4 py-2 bg-primary-500 text-white text-sm font-medium rounded-lg hover:bg-primary-600">
            <Plus className="w-4 h-4 mr-2" />{t.training.add}
          </button>
        )}
        {hasPermission(userRole, 'assign_course') && (
          <button onClick={() => setShowAssignModal(true)} className="flex items-center px-4 py-2 bg-green-500 text-white text-sm font-medium rounded-lg hover:bg-green-600">
            <User className="w-4 h-4 mr-2" />{t.training.assign}
          </button>
        )}
        {hasPermission(userRole, 'request_course') && (
          ...
          >
            <MessageSquarePlus className="w-4 h-4 mr-2" />{t.training.request}
          </button>
        )}
      </div>
          ...
          {courses.map((course) => (
            <div key={course.id} onClick={() => setSelectedCourse(course)} className="bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-lg transition-all cursor-pointer group">
              <div className="h-32 bg-gradient-to-br from-primary-100 to-primary-50 flex items-center justify-center text-5xl relative">
                {course.image_emoji || '📚'}
                {course.is_mandatory && <span className="absolute top-2 right-2 px-2 py-0.5 bg-red-500 text-white text-xs font-medium rounded">{t.training.mandatory}</span>}
                {course.requires_certificate && <span className="absolute top-2 left-2 px-2 py-0.5 bg-purple-500 text-white text-xs font-medium rounded flex items-center gap-1"><FileWarning className="w-3 h-3" />{t.training.certif}</span>}
              ...
              <div className="p-4">
                ...
                <p className="text-xs text-gray-500 mb-3">{course.provider || t.training.internal}</p>
