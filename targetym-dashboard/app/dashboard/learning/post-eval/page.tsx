...
export default function PostEvalPage() {
  const {
    userRole, epfPending, epfAll, epfStats, employees,
  ...
  const { t } = useI18n();
  const tp = t.training.postEvalPage;
  ...
  // Vue employé : ses propres résultats d'évaluation
  if (isEmployee) {
    const myCompleted = epfAll.filter(e => e.status === 'completed');
    const myScores = myCompleted.map(e => e.score).filter(s => s !== null) as number[];
    const avgScore = myScores.length ? Math.round(myScores.reduce((a, b) => a + b, 0) / myScores.length) : 0;
    const validatedCount = myCompleted.filter(e => e.competency_validated).length;
    const threshold = epfStats?.passing_threshold || 70;
      ...
      <div className="space-y-6">
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
          <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-gray-500">{tp.evaluations}</p>
          ...
          <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-gray-500">{tp.avgScore}</p>
          ...
          <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-gray-500">{tp.validationRate}</p>
        ...
        {myCompleted.length === 0 ? (
          <div className="bg-white rounded-xl p-12 text-center">
            ...
            <p className="text-gray-500">{tp.noCompletedEval}</p>
            <p className="text-sm text-gray-400 mt-1">{tp.resultsAppearHere}</p>
          </div>
        ) : (
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500 mt-0.5">{tp.evaluatedBy} {epf.evaluator_name || '—'}</p>
                      {epf.completed_at && <p className="text-xs text-gray-400 mt-0.5">{new Date(epf.completed_at).toLocaleDateString('fr-FR')}</p>}
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    <div className="text-center">
                      <p className={`text-2xl font-bold ${getScoreColor(epf.score || 0, threshold)}`}>{epf.score ?? '—'}/100</p>
                      <p className="text-xs text-gray-400">{tp.score}</p>
                    </div>
                    <span className={`px-3 py-1 rounded-full text-sm font-medium ${getRecommendationColor(epf.recommendation)}`}>{getRecommendationLabel(epf.recommendation)}</span>
    ...
    <div className="space-y-6">
      ...
      {epfStats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between">
              <div><p className="text-xs text-gray-500">{tp.pendingLabel}</p><p className="text-2xl font-bold text-amber-600">{epfStats.pending}</p></div>
          ...
          <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between">
              <div><p className="text-xs text-gray-500">{tp.completedLabel}</p><p className="text-2xl font-bold text-green-600">{epfStats.completed}</p></div>
          ...
          <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between">
              <div><p className="text-xs text-gray-500">{tp.avgScoreLabel}</p><p className="text-2xl font-bold text-primary-600">{epfStats.avg_score}/100</p></div>
          ...
          <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between">
              <div><p className="text-xs text-gray-500">{tp.validationRateLabel}</p><p className="text-2xl font-bold text-primary-600">{epfStats.validation_rate}%</p></div>
              <div className="w-10 h-10 bg-primary-100 rounded-lg flex items-center justify-center"><Award className="w-5 h-5 text-primary-600" /></div>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex gap-2 items-center">
        <button onClick={() => setEpfSubTab('pending')} className={`px-4 py-2 text-sm font-medium rounded-lg ${epfSubTab === 'pending' ? 'bg-primary-500 text-white' : 'bg-white border border-gray-300 text-gray-700'}`}>{tp.pendingLabel} ({epfPending.length})</button>
        <button onClick={() => setEpfSubTab('all')} className={`px-4 py-2 text-sm font-medium rounded-lg ${epfSubTab === 'all' ? 'bg-primary-500 text-white' : 'bg-white border border-gray-300 text-gray-700'}`}>{tp.allTab}</button>
        <button onClick={() => setEpfSubTab('history')} className={`px-4 py-2 text-sm font-medium rounded-lg ${epfSubTab === 'history' ? 'bg-primary-500 text-white' : 'bg-white border border-gray-300 text-gray-700'}`}>{tp.history}</button>
        {hasPermission(userRole, 'view_analytics') && (
          <button onClick={() => { fetchEpfSettings(); setShowEpfSettings(true); }} className="ml-auto p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg" title={tp.epfSettingsLabel}><Settings className="w-5 h-5" /></button>
        )}
      </div>
        ...
        <div className="space-y-4">
          {epfPending.length === 0 ? (
            <div className="bg-white rounded-xl p-12 text-center">
              ...
              <p className="text-gray-500">{tp.noEvalPending}</p>
              <p className="text-sm text-gray-400 mt-1">{tp.evalsAppearHere}</p>
            </div>
          ) : (
            epfPending.map((epf) => (
              <div key={epf.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      {epf.scheduled_date && (<p className="text-xs text-gray-400 mt-1">{tp.scheduled}: {epf.scheduled_date} • {tp.dueDate}: {epf.due_date}</p>)}
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    {epf.evaluator_name ? (
                      <div className="text-right"><p className="text-xs text-gray-500">{tp.evaluatorCol}</p><p className="text-sm font-medium text-gray-700">{epf.evaluator_name}</p></div>
                    ) : (
                      <button onClick={() => { setShowAssignEvaluator(epf); setSelectedEvaluatorId(''); }} className="px-3 py-1.5 bg-gray-100 text-gray-600 text-sm rounded-lg hover:bg-gray-200 flex items-center gap-1"><UserPlus className="w-4 h-4" />{tp.assignLabel}</button>
                    )}
                    <button onClick={() => openEvalModal(epf)} className="px-4 py-2 bg-primary-500 text-white text-sm font-medium rounded-lg hover:bg-primary-600 flex items-center gap-2"><ClipboardCheck className="w-4 h-4" />{tp.evaluate}</button>
                  </div>
                </div>
              </div>
            ))
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">{tp.training}</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">{tp.collaborator}</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">{tp.evaluatorCol}</th>
                  <th className="text-center px-4 py-3 text-xs font-semibold text-gray-500">{tp.score}</th>
                  <th className="text-center px-4 py-3 text-xs font-semibold text-gray-500">{t.common.status}</th>
                  <th className="text-center px-4 py-3 text-xs font-semibold text-gray-500">{tp.recommendation}</th>
                  <th className="text-center px-4 py-3 text-xs font-semibold text-gray-500">{t.common.actions}</th>
                </tr>
              </thead>
                  ...
                  <tr key={epf.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-center">
                      <div className="flex items-center justify-center gap-1">
                        <button onClick={() => setShowEpfDetail(epf)} className="p-1.5 text-gray-400 hover:text-primary-600 hover:bg-primary-50 rounded" title={tp.detail}><Eye className="w-4 h-4" /></button>
                        {epf.status === 'pending' && (<button onClick={() => openEvalModal(epf)} className="p-1.5 text-gray-400 hover:text-green-600 hover:bg-green-50 rounded" title={tp.evaluate}><ClipboardCheck className="w-4 h-4" /></button>)}
                        {epf.status === 'completed' && !epf.career_synced && hasPermission(userRole, 'view_analytics') && (<button onClick={() => setConfirmDialog({ isOpen: true, title: tp.syncCareer, message: tp.syncCareerConfirm, danger: false, onConfirm: async () => { setConfirmDialog(null); await syncCareer(epf.id); } })} className="p-1.5 text-gray-400 hover:text-primary-600 hover:bg-primary-50 rounded" title={tp.syncCareer}><Zap className="w-4 h-4" /></button>)}
                        {epf.career_synced && (<span className="p-1.5 text-green-500" title={tp.synced}><Link className="w-4 h-4" /></span>)}
                      </div>
                    </td>
                  </tr>
                ))}
