...
function calculateSeniority(hireDate: string, t: ReturnType<typeof useI18n>['t']): { years: number; months: number; label: string } {
  ...
  const parts: string[] = [];
  if (years > 0) parts.push(`${years} ${years > 1 ? t.mySpace.career.yearsPlural : t.mySpace.career.years}`);
  if (months > 0) parts.push(`${months} ${t.mySpace.career.months}`);
  if (parts.length === 0) parts.push(t.mySpace.career.lessThanMonth);

  return { years, months, label: parts.join(` ${t.mySpace.career.and} `) };
}
...
function buildCareerTimeline(
  ...
  if (employee.hire_date) {
    events.push({
      ...
      type: 'hire',
      title: t.mySpace.career.startAtCompany,
      subtitle: employee.job_title || t.mySpace.career.initialPosition,
      description: `${employee.department_name ? `${t.mySpace.career.departmentField}: ${employee.department_name}` : ''}${employee.contract_type ? ` • ${employee.contract_type}` : ''}`,
  ...
  salaryHistory.forEach((sh, i) => {
    events.push({
      ...
      type: 'salary_change',
      title: sh.reason || (i === 0 ? t.mySpace.career.initialSalary : t.mySpace.career.salaryReview),
  ...
  completedTrainings.forEach((a) => {
    if (a.completed_at) {
      events.push({
        ...
        type: 'training',
        title: t.mySpace.career.trainingCompleted,
  ...
  evaluations.filter(e => e.status === 'completed').forEach((ev) => {
    events.push({
      ...
      type: 'evaluation',
      title: ev.campaign_name || t.mySpace.career.performanceEvaluation,
      subtitle: ev.overall_score ? `${t.mySpace.career.score}: ${ev.overall_score}/5` : undefined,
      description: ev.evaluator_name ? `${t.mySpace.career.by} ${ev.evaluator_name}` : undefined,
      ...
      if ([1, 2, 3, 5, 10, 15, 20, 25].includes(yearsOfService)) {
        ...
        if (milestoneDate <= new Date().toISOString().substring(0, 10)) {
          events.push({
            ...
            date: milestoneDate,
            type: 'milestone',
            title: `🎉 ${yearsOfService} ${yearsOfService > 1 ? t.mySpace.career.yearsPlural : t.mySpace.career.years} ${t.mySpace.career.seniorityMilestone}`,
...
function SalaryChart({ history, currentSalary, currency }: {
  ...
  const data = useMemo(() => {
    ...
    if (sorted.length === 0 && currentSalary) {
      return [{ date: t.mySpace.career.currentSalaryLabel, amount: currentSalary }];
    }
...
function getFilterOptions(t: ReturnType<typeof useI18n>['t']): { key: FilterType; label: string; types: CareerEvent['type'][] }[] {
  return [
    { key: 'all', label: t.mySpace.career.filterAll, types: [] },
    { key: 'career', label: t.mySpace.career.filterCareer, types: ['hire', 'promotion', 'department_change', 'milestone'] },
    { key: 'salary', label: t.mySpace.career.filterSalary, types: ['salary_change'] },
    { key: 'training', label: t.mySpace.career.filterTrainings, types: ['training', 'certification'] },
    { key: 'evaluation', label: t.mySpace.career.filterEvaluations, types: ['evaluation'] },
    { key: 'mission', label: t.mySpace.career.filterMissions, types: ['mission'] },
  ];
}
...
export default function MyCareerPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (!user.employee_id) {
        setError(t.mySpace.career.accountNotLinked);
        return;
      }
      ...
      if (!emp) {
        setError(t.mySpace.career.profileNotFound);
        return;
      }
      ...
      // Build timeline
      const events = buildCareerTimeline(emp, safeAssignments, safeMissions, safeEvaluations, safeSalHistory, t);
      ...
      console.error(err);
      setError(t.mySpace.career.loadingError);
    } finally {
      setLoading(false);
    }
  }, [t]);
  ...
  // Filtered events
  const FILTER_OPTIONS = useMemo(() => getFilterOptions(t), [t]);
  ...
  }, [timelineEvents, activeFilter, FILTER_OPTIONS]);
  ...
  const seniority = useMemo(() => {
    ...
    return calculateSeniority(employee.hire_date, t);
  }, [employee, t]);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500">{t.mySpace.career.loadingCareer}</p>
        </div>
      </div>
    );
  }
    ...
    <>
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden mb-6">
          <div className="bg-primary-600 px-4 sm:px-6 py-5 sm:py-8">
            <div className="flex items-start sm:items-center gap-3 sm:gap-5">
              <div className="w-14 sm:w-20 h-14 sm:h-20 bg-white/20 rounded-2xl flex items-center justify-center text-white text-lg sm:text-2xl font-bold border-2 border-white/30 shrink-0">
                {employee.first_name?.[0]}{employee.last_name?.[0]}
              </div>
              <div className="text-white min-w-0 flex-1">
                <h1 className="text-lg sm:text-2xl font-bold text-white">{employee.first_name} {employee.last_name}</h1>
                <p className="text-white text-sm sm:text-lg font-medium">{employee.job_title || '—'}</p>
                <div className="flex flex-col sm:flex-row sm:items-center sm:gap-4 gap-1 mt-2 text-white text-xs sm:text-sm">
                  ...
                  {employee.contract_type && (
                    <span className="flex items-center gap-1">
                      <Briefcase className="w-3.5 sm:w-4 h-3.5 sm:h-4 shrink-0" />{employee.contract_type}
                    </span>
                  )}
          ...
          {seniority && (
            <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="text-sm font-medium text-gray-900">{t.mySpace.career.seniorityLabel}</span>
                  <span className="text-sm text-primary-600 font-semibold">{seniority.label}</span>
                </div>
              </div>
              {employee.hire_date && (
                <span className="text-sm text-gray-500">
                  {t.mySpace.career.since} {formatDate(employee.hire_date)}
                </span>
              )}
            </div>
          )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-6" data-tour="skills-analysis">
          ...
          <StatCard
            ...
            label={t.mySpace.career.evaluations}
            value={evaluationCount}
            sublabel={avgScore ? `${t.mySpace.career.average}: ${avgScore.toFixed(1)}/5` : undefined}
            color="bg-amber-50"
          ...
          {currentSalaryValue && (
            <StatCard
              ...
              label={t.mySpace.career.currentSalary}
        ...
        <div className="grid lg:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5" data-tour="career-path">
            <h3 className="text-sm font-semibold text-gray-700 mb-4 flex items-center gap-2">
              ...
              {t.mySpace.career.currentPosition}
            </h3>
            <div className="space-y-3">
              <div className="flex items-center justify-between py-2 border-b border-gray-50">
                <span className="text-sm text-gray-500">{t.mySpace.career.titleField}</span>
                <span className="text-sm font-medium text-gray-900">{employee.job_title || '—'}</span>
              </div>
              <div className="flex items-center justify-between py-2 border-b border-gray-50">
                <span className="text-sm text-gray-500">{t.mySpace.career.departmentField}</span>
                <span className="text-sm font-medium text-gray-900">{employee.department_name || '—'}</span>
              </div>
              <div className="flex items-center justify-between py-2 border-b border-gray-50">
                <span className="text-sm text-gray-500">{t.mySpace.career.contractType}</span>
                <span className="text-sm font-medium text-gray-900">{employee.contract_type || '—'}</span>
              </div>
              <div className="flex items-center justify-between py-2 border-b border-gray-50">
                <span className="text-sm text-gray-500">{t.mySpace.career.managerField}</span>
                <span className="text-sm font-medium text-gray-900">{employee.manager_name || '—'}</span>
              </div>
              <div className="flex items-center justify-between py-2">
                <span className="text-sm text-gray-500">{t.mySpace.career.statusField}</span>
                <span className={`text-xs font-medium px-2 py-1 rounded-full ${
                  employee.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'
                }`}>
                  {employee.status === 'active' ? t.mySpace.career.active : employee.status || '—'}
                </span>
              </div>
            </div>
          </div>
          ...
          {salaryHistory.length === 0 && !currentSalaryValue && (
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              <h3 className="text-sm font-semibold text-gray-700 mb-4 flex items-center gap-2">
                ...
                {t.mySpace.career.quickLinks}
              </h3>
              <div className="space-y-2">
                {[
                  { label: t.mySpace.career.myTrainings, href: '/dashboard/learning', icon: <GraduationCap className="w-4 h-4" />, color: 'text-indigo-600 bg-indigo-50' },
                  { label: t.mySpace.career.myMissions, href: '/dashboard/missions', icon: <Plane className="w-4 h-4" />, color: 'text-sky-600 bg-sky-50' },
                  { label: t.mySpace.career.myEvaluations, href: '/dashboard/performance/evaluations', icon: <Star className="w-4 h-4" />, color: 'text-amber-600 bg-amber-50' },
                  { label: t.mySpace.career.myObjectives, href: '/dashboard/my-space/objectives', icon: <Target className="w-4 h-4" />, color: 'text-primary-600 bg-primary-50' },
                ].map(link => (
                  <Link 
                    key={link.href} 
                    href={link.href}
        ...
        <div className="mb-6">
          ...
          {filteredEvents.length === 0 ? (
            <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
              ...
              <p className="text-gray-500">{t.mySpace.career.noEventsInCategory}</p>
            </div>
          ) : (
