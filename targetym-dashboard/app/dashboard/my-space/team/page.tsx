  ...
  return (
    ...
    >
      ...
      <span className={`px-2 py-1 rounded-full text-xs font-medium ${
        ...
        {member.status === 'active' ? t.mySpace.activeStatus : member.status === 'on_leave' ? t.mySpace.onLeave : member.status}
      </span>
    </div>
  );
}
    ...
    <div className="bg-white border border-yellow-200 rounded-lg p-4 shadow-sm">
      <div className="flex items-start justify-between">
        <div className="flex-1">
          ...
          <p className="text-xs text-gray-400 mt-2">{t.mySpace.requestedOn} {new Date(request.created_at).toLocaleDateString('fr-FR')}</p>
        </div>
        <div className="flex items-center gap-2 ml-4">
          <button onClick={onApprove} disabled={processing} className="p-2 text-green-600 hover:bg-green-100 rounded-lg transition-colors disabled:opacity-50" title={t.mySpace.approveTitle}><CheckCircle className="w-6 h-6" /></button>
          <button onClick={() => setShowRejectForm(!showRejectForm)} disabled={processing} className="p-2 text-red-600 hover:bg-red-100 rounded-lg transition-colors disabled:opacity-50" title={t.mySpace.refuseTitle}><XCircle className="w-6 h-6" /></button>
        </div>
      </div>
      {showRejectForm && (
        <div className="mt-4 pt-4 border-t border-gray-200">
          <label className="block text-sm font-medium text-gray-700 mb-2">{t.mySpace.rejectReasonLabel}</label>
          <textarea value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-red-500 focus:border-red-500" rows={2} placeholder={t.mySpace.rejectReasonPlaceholder} />
          <div className="flex gap-2 mt-2">
            <button onClick={() => setShowRejectForm(false)} className="px-3 py-1.5 text-sm text-gray-600 hover:bg-gray-100 rounded-lg">{t.common.cancel}</button>
            <button onClick={() => { if (!rejectReason.trim()) { toast.error(t.mySpace.indicateReason); return; } onReject(rejectReason); }} disabled={processing} className="px-3 py-1.5 text-sm bg-red-600 text-white rounded-lg hover:bg-red-700 disabled:opacity-50">{t.mySpace.confirmRefuse}</button>
          </div>
        </div>
      )}
    </div>
...
function MemberDetailPanel({ member, onClose }: { member: Employee; onClose: () => void }) {
  ...
  const tabs = [
    { id: 'perf' as const, label: t.mySpace.performanceTab, icon: TrendingUp },
    { id: 'conges' as const, label: t.mySpace.leavesTab, icon: Palmtree },
    { id: 'taches' as const, label: t.mySpace.tasksTab, icon: ListTodo },
    { id: 'entretiens' as const, label: t.mySpace.interviewsTab, icon: MessageSquare },
  ];
            ...
            {activeTab === 'perf' && (
              <div className="space-y-4">
                {perfScore ? (
                  ...
                  <p className="text-center text-sm text-gray-400 py-4">{t.mySpace.noPerformanceData}</p>
                )}

                {okrs.length > 0 && (
                  <div>
                    <h4 className="text-sm font-semibold text-gray-700 mb-2 flex items-center gap-1.5"><Target className="w-3.5 h-3.5" />{t.mySpace.objectivesLabel}</h4>
            ...
            {activeTab === 'conges' && (
              <div className="space-y-4">
                {leaveBalances.length > 0 ? (
                  <div>
                    <h4 className="text-sm font-semibold text-gray-700 mb-2">{t.mySpace.leaveBalanceLabel}</h4>
                    <div className="space-y-2">
                      {leaveBalances.map((b, i) => (
                        <div key={i} className="flex items-center justify-between bg-gray-50 rounded-lg p-2.5">
                          ...
                          <div className="flex items-center gap-3 text-xs">
                            <span className="text-green-600 font-medium">{b.available}j {t.mySpace.availableShort}</span>
                            <span className="text-gray-400">{b.taken}j {t.mySpace.takenShort}</span>
                            {b.pending > 0 && <span className="text-yellow-600">{b.pending}j {t.mySpace.pendingShort}</span>}
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <p className="text-center text-sm text-gray-400 py-2">{t.mySpace.noLeaveBalance}</p>
                )}

                {leaveRequests.length > 0 && (
                  <div>
                    <h4 className="text-sm font-semibold text-gray-700 mb-2">{t.mySpace.lastRequests}</h4>
                    <div className="space-y-2">
                      {leaveRequests.map(r => (
            ...
            {activeTab === 'taches' && (
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  {[
                    { label: t.mySpace.totalLabel, value: taskData.stats.total, color: 'text-gray-700' },
                    { label: t.mySpace.inProgressTasks, value: taskData.stats.in_progress, color: 'text-blue-600' },
                    { label: t.mySpace.completedTasks, value: taskData.stats.completed, color: 'text-green-600' },
                    { label: t.mySpace.overdueTasks, value: taskData.stats.overdue, color: 'text-red-600' },
                  ].map(s => (
                ...
                {taskData.stats.total > 0 && (
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-xs text-gray-500">{t.mySpace.completionRate}</span>
                ...
                {taskData.tasks.length > 0 ? (
                    ...
                    {taskData.tasks.map(task => (
                      <div key={task.id} className="flex items-center justify-between bg-gray-50 rounded-lg p-2.5">
                        <div className="flex-1 min-w-0 mr-2">
                          ...
                          <p className="text-xs text-gray-400">{formatDate(task.due_date)} {task.is_overdue && <span className="text-red-500 font-medium">{t.mySpace.overdueTasks}</span>}</p>
                        </div>
                  ...
                  <p className="text-center text-sm text-gray-400 py-4">{t.mySpace.noTaskAssigned}</p>
                )}
              </div>
            )}
            ...
            {activeTab === 'entretiens' && (
              <div className="space-y-3">
                {oneOnOnes.length > 0 ? (
                    ...
                    <div key={o.id} className="bg-gray-50 rounded-lg p-3">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                          ...
                          {o.status === 'scheduled' ? t.mySpace.planned : o.status === 'completed' ? t.mySpace.completedInterview : o.status}
                        </span>
                      </div>
                  ...
                  <p className="text-center text-sm text-gray-400 py-4">{t.mySpace.noInterviewPlanned}</p>
                )}
              </div>
            )}
          </>
...
export default function MyTeamPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (!user.employee_id) { setError(t.mySpace.accountNotLinked); return; }
  ...
  const handleApprove = async (requestId: number) => {
    ...
    catch { toast.error(t.mySpace.approvalError); }
  ...
  const handleReject = async (requestId: number, reason: string) => {
    ...
    catch { toast.error(t.mySpace.refusalError2); }
    finally { setProcessingId(null); }
  };
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8" data-tour="team-stats">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
              <div className="flex items-center gap-3">
                ...
                <div><p className="text-2xl font-bold text-gray-900">{teamMembers.length}</p><p className="text-sm text-gray-500">{t.mySpace.collaborators}</p></div>
              </div>
            </div>
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
              <div className="flex items-center gap-3">
                ...
                <div><p className="text-2xl font-bold text-gray-900">{pendingRequests.length}</p><p className="text-sm text-gray-500">{t.mySpace.pendingLeaveRequests}</p></div>
              </div>
            </div>
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
              <div className="flex items-center gap-3">
                ...
                <div><p className="text-2xl font-bold text-gray-900">{teamMembers.filter(m => m.status === 'active').length}</p><p className="text-sm text-gray-500">{t.mySpace.activeToday}</p></div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h2 className="text-lg font-semibold text-gray-900">{t.mySpace.pendingLeaveRequests} ({pendingRequests.length})</h2>
            </div>
            {pendingRequests.length === 0 ? (
              <div className="text-center py-8 text-gray-500">
                <CheckCircle className="w-12 h-12 mx-auto mb-3 text-green-300" />
                <p>{t.mySpace.noPendingRequest}</p>
              </div>
            ) : (
              <div className="space-y-4">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <button onClick={() => setShowTeam(!showTeam)} className="w-full flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-lg font-semibold text-gray-900">{t.mySpace.myTeamCount} ({teamMembers.length})</h2>
                </div>
