...
function getCategoryConfig(t: any): Record<string, { label: string; icon: any; color: string }> {
  return {
    administratif: { label: t.onboarding.categoryAdmin, icon: FileText, color: 'text-blue-600 bg-blue-50' },
    materiel: { label: t.onboarding.categoryMaterial, icon: Monitor, color: 'text-purple-600 bg-purple-50' },
    formation: { label: t.onboarding.categoryTraining, icon: GraduationCap, color: 'text-green-600 bg-green-50' },
    rencontre: { label: t.onboarding.categoryMeeting, icon: Handshake, color: 'text-amber-600 bg-amber-50' },
    documentation: { label: t.onboarding.categoryDocumentation, icon: BookOpen, color: 'text-indigo-600 bg-indigo-50' },
    acces_it: { label: t.onboarding.categoryIT, icon: ShieldCheck, color: 'text-cyan-600 bg-cyan-50' },
    general: { label: t.onboarding.categoryGeneral, icon: ClipboardList, color: 'text-gray-600 bg-gray-50' },
  };
}

function getRoleConfig(t: any): Record<string, { label: string; color: string }> {
  return {
    hr: { label: t.onboarding.roleHR, color: 'bg-blue-100 text-blue-700' },
    manager: { label: t.onboarding.roleManager, color: 'bg-green-100 text-green-700' },
    it: { label: t.onboarding.roleIT, color: 'bg-purple-100 text-purple-700' },
    employee: { label: t.onboarding.roleEmployee, color: 'bg-amber-100 text-amber-700' },
    buddy: { label: t.onboarding.roleBuddy, color: 'bg-pink-100 text-pink-700' },
  };
}

function getStatusConfig(t: any): Record<string, { label: string; color: string; bg: string }> {
  return {
    not_started: { label: t.onboarding.statusNotStarted, color: 'text-gray-700', bg: 'bg-gray-100' },
    in_progress: { label: t.onboarding.statusInProgress, color: 'text-blue-700', bg: 'bg-blue-100' },
    completed: { label: t.onboarding.statusCompleted, color: 'text-green-700', bg: 'bg-green-100' },
    cancelled: { label: t.onboarding.statusCancelled, color: 'text-red-700', bg: 'bg-red-100' },
    overdue: { label: t.onboarding.statusOverdue, color: 'text-orange-700', bg: 'bg-orange-100' },
    pending: { label: t.onboarding.statusPending, color: 'text-yellow-700', bg: 'bg-yellow-100' },
    skipped: { label: t.onboarding.statusSkipped, color: 'text-gray-500', bg: 'bg-gray-50' },
    blocked: { label: t.onboarding.statusBlocked, color: 'text-red-700', bg: 'bg-red-100' },
    scheduled: { label: t.onboarding.statusScheduled, color: 'text-blue-700', bg: 'bg-blue-100' },
    confirmed: { label: t.onboarding.statusConfirmed, color: 'text-green-700', bg: 'bg-green-100' },
  };
}

function getTaskStatusConfig(t: any): Record<string, { label: string; color: string; bg: string }> {
  return {
    pending: { label: t.onboarding.taskPending, color: 'text-yellow-700', bg: 'bg-yellow-100' },
    in_progress: { label: t.onboarding.taskInProgress, color: 'text-blue-700', bg: 'bg-blue-100' },
    completed: { label: t.onboarding.taskCompleted, color: 'text-green-700', bg: 'bg-green-100' },
    skipped: { label: t.onboarding.taskSkipped, color: 'text-gray-500', bg: 'bg-gray-50' },
    blocked: { label: t.onboarding.taskBlocked, color: 'text-red-700', bg: 'bg-red-100' },
  };
}
...
function StatusBadge({ status, config }: { status: string; config: Record<string, { label: string; color: string; bg: string }> }) {
  const cfg = config[status] || { label: status, color: 'text-gray-700', bg: 'bg-gray-100' };
...
function SearchableSelect({
  options, value, onChange, placeholder, emptyLabel, className = ''
}: {
...
export default function OnboardingPage() {
  const { t } = useI18n();
  const dateLocale = t.dashboard.dateLocale || 'fr-FR';
  const CATEGORY_CONFIG = getCategoryConfig(t);
  const ROLE_CONFIG = getRoleConfig(t);
  const STATUS_CONFIG = getStatusConfig(t);
  const TASK_STATUS_CONFIG = getTaskStatusConfig(t);
  ...
  const handleArchiveProgram = async (p: Program) => {
    ...
    setConfirmDialog({
      isOpen: true,
      title: willArchive ? t.onboarding.archiveProgram : t.onboarding.restoreProgram,
      message: willArchive
        ? `${t.onboarding.archive} "${p.name}" ${t.onboarding.archiveMessage}`
        : `${t.onboarding.restore} "${p.name}" ${t.onboarding.restoreMessage}`,
      danger: willArchive,
      onConfirm: async () => {
        ...
        try {
          ...
          toast.success(willArchive ? t.onboarding.programArchived : t.onboarding.programRestored);
          fetchPrograms();
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4" data-tour="onboarding-stats">
          <StatCard label={t.onboarding.totalOnboardings} value={stats.total} icon={Users} color="bg-primary-50 text-primary-600" />
          <StatCard label={t.onboarding.inProgress} value={stats.in_progress} icon={Rocket} color="bg-amber-50 text-amber-600" sub={`${stats.avg_progress}% ${t.onboarding.avgProgress}`} />
          <StatCard label={t.onboarding.completed} value={stats.completed} icon={CheckCircle} color="bg-green-50 text-green-600" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200">
            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
              <h3 className="font-semibold text-gray-900">{t.onboarding.activeOnboardings}</h3>
              <span className="text-xs text-gray-400">{active_onboardings.length} {t.onboarding.inProgressCount}</span>
            </div>
            <div className="divide-y divide-gray-50">
              {active_onboardings.length === 0 ? (
                <EmptyState icon={Users} title={t.onboarding.noActiveOnboarding} />
                ...
                <div key={ob.id} className="px-5 py-3 hover:bg-gray-50 cursor-pointer" onClick={() => { setActiveTab('suivi'); fetchAssignmentDetail(ob.id); }}>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <StatusBadge status={ob.status} config={STATUS_CONFIG} />
                  </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl border border-gray-200">
              <div className="px-5 py-4 border-b border-gray-100 flex items-center gap-2">
                ...
                <h3 className="font-semibold text-gray-900">{t.onboarding.overdueTasks}</h3>
              ...
              <div className="divide-y divide-gray-50 max-h-48 overflow-y-auto">
                {overdue_tasks.length === 0 ? (
                  <p className="text-sm text-gray-400 text-center py-6">{t.onboarding.noOverdueTask}</p>
                ) : overdue_tasks.map(ot => (
                  <div key={ot.id} className="px-5 py-2.5 flex items-center justify-between hover:bg-red-50/30">
                    <div>
                      <p className="text-sm font-medium">{ot.title}</p>
                      <p className="text-xs text-gray-400">{ot.employee_name} • {t.onboarding.dueOn} {formatDate(ot.due_date, dateLocale)}</p>
                    </div>
                    <span className={`text-xs px-2 py-0.5 rounded-full ${ROLE_CONFIG[ot.assigned_role]?.color || 'bg-gray-100'}`}>
                      {ROLE_CONFIG[ot.assigned_role]?.label || ot.assigned_role}
                    </span>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200">
              <div className="px-5 py-4 border-b border-gray-100 flex items-center gap-2">
                ...
                <h3 className="font-semibold text-gray-900">{t.onboarding.upcomingGetToKnow}</h3>
              </div>
              <div className="divide-y divide-gray-50 max-h-48 overflow-y-auto">
                {upcoming_get_to_know.length === 0 ? (
                  <p className="text-sm text-gray-400 text-center py-6">{t.onboarding.noMeetingPlanned}</p>
                  ...
                  <div key={g.id} className="px-5 py-2.5 hover:bg-amber-50/30">
                    <div className="flex items-center justify-between">
                      ...
                      <StatusBadge status={g.status} config={STATUS_CONFIG} />
                    </div>
                    <div className="flex items-center gap-2 text-xs text-gray-400 mt-0.5">
                      <Calendar size={12} /> {formatDate(g.scheduled_date, dateLocale)}
  ...
  const renderProgrammes = () => {
    ...
    return (
      <div className="space-y-4" data-tour="onboarding-programs">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-semibold">{t.onboarding.integrationPrograms}</h3>
          <div className="flex items-center gap-2">
            <button
            ...
            >
              <Archive size={15} />
              {showArchived ? t.onboarding.viewActive : t.onboarding.viewArchived}
            </button>
            {isHR && !showArchived && (
              <button onClick={() => { setEditingProgram(null); setShowProgramModal(true); }}
                ...
                <Plus size={16} /> {t.onboarding.newProgram}
              </button>
            )}
          </div>
        </div>
        ...
        {programs.length === 0 ? (
          <EmptyState icon={showArchived ? Archive : ClipboardList} title={showArchived ? t.onboarding.noArchivedProgram : t.onboarding.noProgram} subtitle={showArchived ? t.onboarding.archivedProgramsHere : t.onboarding.createFirstProgram} />
        ) : (
            ...
            {programs.map(p => (
              <div key={p.id}
                ...
                <div className="flex items-start justify-between mb-3 pr-6">
                  ...
                  <div className="flex items-center gap-2">
                    {p.is_default && <span className="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full">{t.onboarding.defaultProgram}</span>}
                    {!p.is_active && <span className="text-xs bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full flex items-center gap-1"><Archive size={10} /> {t.onboarding.archived}</span>}
                  </div>
                </div>
                {p.description && <p className="text-sm text-gray-500 line-clamp-2 mb-3">{p.description}</p>}
                <div className="flex items-center gap-4 text-xs text-gray-400">
                  <span className="flex items-center gap-1"><ClipboardList size={13} /> {p.task_count} {t.onboarding.tasks}</span>
                  <span className="flex items-center gap-1"><Calendar size={13} /> {p.duration_days} {t.onboarding.days}</span>
                  <span className="flex items-center gap-1"><Users size={13} /> {p.usage_count} {p.usage_count > 1 ? t.onboarding.usedPlural : t.onboarding.used}</span>
                </div>
                </div>
              </div>
            ))}
    ...
    return (
      <div className="space-y-4">
        <button onClick={() => setSelectedProgram(null)} className="flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700">
          <ArrowLeft size={16} /> {t.onboarding.backToPrograms}
        </button>
          ...
          <div className="flex items-start justify-between mb-4">
            <div>
              ...
              <div className="flex items-center gap-4 mt-2 text-xs text-gray-400">
                ...
                <span className="flex items-center gap-1"><Calendar size={12} /> {p.duration_days} {t.onboarding.days}</span>
                <span className="flex items-center gap-1"><ClipboardList size={12} /> {(p.tasks || []).length} {t.onboarding.tasks}</span>
              </div>
            </div>
            {isHR && (
              <div className="flex gap-2">
                ...
                >
                  {p.is_active ? <><Archive size={14} /> {t.onboarding.archive}</> : <><ArchiveRestore size={14} /> {t.onboarding.restore}</>}
                </button>
                {p.is_active && (
                  <>
                    <button onClick={() => { setEditingProgram(p); setShowProgramModal(true); }}
                      className="flex items-center gap-1 px-3 py-1.5 text-sm border rounded-lg hover:bg-gray-50"><Edit size={14} /> {t.common.edit}</button>
                    <button onClick={() => { setEditingTask(null); setShowTaskModal(true); }}
                      className="flex items-center gap-1 px-3 py-1.5 text-sm bg-primary-600 text-white rounded-lg hover:bg-primary-700"><Plus size={14} /> {t.onboarding.task}</button>
                  </>
                )}
              </div>
            )}
          ...
          return (
            <div key={cat} className="bg-white rounded-xl border border-gray-200">
              <div className={`px-5 py-3 border-b border-gray-100 flex items-center gap-2`}>
                ...
                <span className="text-xs text-gray-400 ml-auto">{tasks.length} {t.onboarding.tasks}</span>
              </div>
              <div className="divide-y divide-gray-50">
                {tasks.sort((a, b) => a.due_day - b.due_day).map(tk => (
                  <div key={tk.id} className="px-5 py-3 flex items-center justify-between hover:bg-gray-50">
                    <div className="flex-1">
                      <p className="text-sm font-medium">{tk.title}</p>
                      {tk.description && <p className="text-xs text-gray-400 mt-0.5 line-clamp-1">{tk.description}</p>}
                    </div>
                    <div className="flex items-center gap-3 text-xs ml-4">
                      <span className="text-gray-400">J+{tk.due_day}</span>
                      <span className={`px-2 py-0.5 rounded-full ${ROLE_CONFIG[tk.assigned_role]?.color || 'bg-gray-100'}`}>
                        {ROLE_CONFIG[tk.assigned_role]?.label || tk.assigned_role}
                      </span>
                      {tk.is_required && <span className="text-red-400" title={t.onboarding.required}>*</span>}
                      {tk.requires_document && <span className="text-gray-400" title={t.onboarding.documentRequired}><FileText size={13} /></span>}
                      {isHR && (
                        <div className="flex items-center gap-1">
                          <button onClick={(e) => { e.stopPropagation(); setEditingTask(tk); setShowTaskModal(true); }}
                            className="text-gray-300 hover:text-primary-500"><Edit size={14} /></button>
                          <button onClick={async (e) => {
                            e.stopPropagation();
                            setConfirmDialog({
                              isOpen: true,
                              title: t.onboarding.deleteTask,
                              message: t.onboarding.deleteTaskConfirm,
                              danger: true,
                              onConfirm: async () => {
                                setConfirmDialog(null);
                                await apiFetch(`/api/onboarding/tasks/${tk.id}`, { method: 'DELETE' });
    ...
    return (
      <div className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <h3 className="text-lg font-semibold">{t.onboarding.trackingTitle}</h3>
          </div>
          <div className="flex items-center gap-2">
            <div className="relative">
              <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
              <input value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
                placeholder={`${t.common.search}...`} className="pl-9 pr-4 py-2 border rounded-lg text-sm w-56" />
            </div>
            {isHR && (
              <button onClick={() => setShowAddDropdown(!showAddDropdown)}
                className="flex items-center gap-2 px-4 py-2 bg-primary-600 text-white text-sm rounded-lg hover:bg-primary-700">
                <Plus size={16} /> {t.common.create} <ChevronDown size={14} />
              </button>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {[
            { value: 'all', label: t.onboarding.all },
            { value: 'not_started', label: t.onboarding.notStarted },
        ...
        {filteredAssignments.length === 0 ? (
          <EmptyState icon={Users} title={t.onboarding.noOnboardingFound} />
        ) : (
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b">
                <tr>
                  <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase">{t.onboarding.employee}</th>
                  <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase">{t.onboarding.program}</th>
                  <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase">{t.onboarding.progression}</th>
                  <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase">{t.onboarding.start}</th>
                  <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase">{t.onboarding.status}</th>
                  <th className="text-right px-5 py-3 text-xs font-semibold text-gray-500 uppercase">{t.onboarding.actions}</th>
                </tr>
              </thead>
                ...
                {filteredAssignments.map(a => (
                  <tr key={a.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => fetchAssignmentDetail(a.id)}>
                    ...
                    <td className="px-5 py-3 w-40">
                      ...
                      <p className="text-xs text-gray-400 mt-0.5">{a.completed_tasks}/{a.total_tasks} {t.onboarding.tasks}</p>
                    </td>
                    <td className="px-5 py-3 text-gray-500 text-xs">{formatDate(a.start_date, dateLocale)}</td>
                    <td className="px-5 py-3"><StatusBadge status={a.status} config={STATUS_CONFIG} /></td>
    ...
    return (
      <div className="space-y-4">
        <button onClick={() => setSelectedAssignment(null)} className="flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700">
          <ArrowLeft size={16} /> {t.onboarding.backToTracking}
        </button>
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <div className="flex flex-wrap items-center gap-4 mt-3 text-xs text-gray-400">
                ...
                {a.hire_date && <span className="flex items-center gap-1"><Calendar size={12} /> {t.onboarding.hireDate}: {formatDate(a.hire_date, dateLocale)}</span>}
                {a.manager_name && <span className="flex items-center gap-1"><UserCheck size={12} /> {t.onboarding.manager}: {a.manager_name}</span>}
                {a.buddy_name && <span className="flex items-center gap-1"><Handshake size={12} /> {t.onboarding.buddy}: {a.buddy_name}</span>}
              </div>
            </div>
            <div className="text-right">
              ...
              <div className="mt-2 w-48">
                <div className="flex items-center justify-between text-xs mb-1">
                  <span className="text-gray-400">{t.onboarding.progression}</span>
                  <span className="font-semibold">{a.progress_percentage}%</span>
                </div>
                <ProgressBar value={a.progress_percentage} />
                <p className="text-xs text-gray-400 mt-1">{a.completed_tasks}/{a.total_tasks} {t.onboarding.tasksDone} • {a.program_name}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {Object.entries(tasksByCategory).map(([cat, tasks]) => {
          ...
          return (
            <div key={cat} className="bg-white rounded-xl border border-gray-200">
              ...
              <div className="divide-y divide-gray-50">
                {tasks.sort((x, y) => (x.due_day || 0) - (y.due_day || 0)).map(tk => {
                  const isOverdue = tk.due_date && tk.status !== 'completed' && new Date(tk.due_date) < new Date();
                  return (
                    <div key={tk.id} className={`px-5 py-3 flex items-center gap-3 ${isOverdue ? 'bg-red-50/40' : 'hover:bg-gray-50'}`}>
                      {/* Checkbox */}
                      <button
                        onClick={() => updateTaskStatus(tk.id, tk.status === 'completed' ? 'pending' : 'completed')}
                        className={`flex-shrink-0 w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${
                          tk.status === 'completed' ? 'bg-green-500 border-green-500 text-white' : 'border-gray-300 hover:border-primary-400'
                        }`}>
                        {tk.status === 'completed' && <CheckCircle size={14} />}
                      </button>

                      <div className="flex-1 min-w-0">
                        <p className={`text-sm font-medium ${tk.status === 'completed' ? 'line-through text-gray-400' : ''}`}>{tk.title}</p>
                        <div className="flex items-center gap-3 text-xs text-gray-400 mt-0.5">
                          <span className={`px-1.5 py-0.5 rounded ${ROLE_CONFIG[tk.assigned_role]?.color || 'bg-gray-100'}`}>
                            {ROLE_CONFIG[tk.assigned_role]?.label || tk.assigned_role}
                          </span>
                          {tk.due_date && <span className={isOverdue ? 'text-red-500 font-medium' : ''}>{t.onboarding.dueDateLabel} {formatDate(tk.due_date, dateLocale)}</span>}
                          {tk.completed_by_name && <span className="text-green-500">✓ {tk.completed_by_name}</span>}
                          {tk.is_required && <span className="text-red-400">{t.onboarding.required}</span>}
                        </div>
                      </div>
        ...
        {a.get_to_know_meetings && a.get_to_know_meetings.length > 0 && (
          <div className="bg-white rounded-xl border border-gray-200">
            <div className="px-5 py-3 border-b border-gray-100 flex items-center gap-2">
              <Handshake size={16} className="text-amber-500" />
              <h4 className="font-semibold text-sm">{t.onboarding.getToKnowMeetings}</h4>
            </div>
            <div className="divide-y divide-gray-50">
              {a.get_to_know_meetings.map(m => (
                <div key={m.id} className="px-5 py-3 flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium">{t.onboarding.meetingWith} {m.meet_employee_name}</p>
                    <p className="text-xs text-gray-400">{m.meet_job_title} • {m.meet_department}</p>
                    <div className="flex items-center gap-2 text-xs text-gray-400 mt-1">
                      <Calendar size={12} /> {formatDate(m.scheduled_date, dateLocale)}
                      {m.scheduled_time && <><Clock size={12} /> {formatTime(m.scheduled_time)}</>}
                      {m.location && <><MapPin size={12} /> {m.location}</>}
                    </div>
                  </div>
                  <StatusBadge status={m.status} config={STATUS_CONFIG} />
                </div>
              ))}
            </div>
          </div>
  ...
  const renderGetToKnow = () => (
    <div className="space-y-4" data-tour="onboarding-meetings">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold">{t.onboarding.getToKnowTitle}</h3>
        {isHR && (
          <button onClick={() => setShowGTKModal(true)}
            ...
            <Plus size={16} /> {t.onboarding.planMeeting}
          </button>
        )}
      </div>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {[
          { value: 'upcoming', label: t.onboarding.upcoming },
          { value: 'all', label: t.onboarding.all },
          { value: 'completed', label: t.onboarding.completed },
          { value: 'cancelled', label: t.onboarding.cancelled },
        ].map(f => (
      ...
      {gtkMeetings.length === 0 ? (
        <EmptyState icon={Handshake} title={t.onboarding.noMeeting} subtitle={t.onboarding.planGetToKnow} />
      ) : (
          ...
          {gtkMeetings.map(g => (
            <div key={g.id} className="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-sm transition-shadow">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400">{t.onboarding.meets}</p>
              ...
              <div className="flex flex-wrap items-center gap-3 text-xs text-gray-400 mb-3">
                <span className="flex items-center gap-1"><Calendar size={12} /> {formatDate(g.scheduled_date, dateLocale)}</span>
              ...
              {g.meeting_link && (
                <a href={g.meeting_link} target="_blank" rel="noopener noreferrer"
                  ...
                  <Link2 size={12} /> {t.onboarding.meetingLink}
                </a>
              )}
            </div>
          ))}
      ...
      <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
          <div className="px-6 py-4 border-b flex items-center justify-between">
            <h3 className="font-semibold">{editingProgram ? t.onboarding.editProgram : t.onboarding.newProgramTitle}</h3>
            <button onClick={() => setShowProgramModal(false)} className="text-gray-400 hover:text-gray-600"><X size={20} /></button>
          </div>
          <div className="p-6 space-y-4">
            <div>
              <label className="text-sm font-medium text-gray-700 mb-1 block">{t.onboarding.programName} *</label>
              <input value={name} onChange={e => setName(e.target.value)} className="w-full border rounded-lg px-3 py-2 text-sm" placeholder={t.onboarding.programNamePlaceholder} />
            </div>
            <div>
            ...
            <div>
              <label className="text-sm font-medium text-gray-700 mb-1 block">{t.onboarding.targetPosition}</label>
              <input value={jobTitle} onChange={e => setJobTitle(e.target.value)} className="w-full border rounded-lg px-3 py-2 text-sm" placeholder={t.onboarding.optionalLabel} />
            </div>
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={isDefault} onChange={e => setIsDefault(e.target.checked)} className="rounded" />
              {t.onboarding.defaultProgramLabel}
            </label>
          </div>
          <div className="px-6 py-4 border-t flex justify-end gap-2">
            <button onClick={() => setShowProgramModal(false)} className="px-4 py-2 text-sm border rounded-lg hover:bg-gray-50">{t.common.cancel}</button>
            <button onClick={handleSave} disabled={saving || !name.trim()} className="px-4 py-2 text-sm bg-primary-600 text-white rounded-lg hover:bg-primary-700 disabled:opacity-50">
              {saving ? <Loader2 size={16} className="animate-spin" /> : editingProgram ? t.common.edit : t.common.create}
            </button>
          </div>
        </div>
      </div>
  ...
  const TaskModal = () => {
    ...
    return (
      <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
          <div className="px-6 py-4 border-b flex items-center justify-between">
            <h3 className="font-semibold">{editingTask ? t.onboarding.editTask : t.onboarding.addTask}</h3>
            <button onClick={() => { setShowTaskModal(false); setEditingTask(null); }} className="text-gray-400 hover:text-gray-600"><X size={20} /></button>
          </div>
          <div className="p-6 space-y-4">
            <div>
              <label className="text-sm font-medium text-gray-700 mb-1 block">{t.onboarding.taskTitle} *</label>
        ...
        <div className="bg-white rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          {queueStartItem && (
            <div className="mx-6 mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg flex items-center gap-2 text-sm text-blue-800">
              ...
              <span>{t.onboarding.forLabel} : <strong>{queueStartItem.candidate_name}</strong> — {queueStartItem.job_title}</span>
            </div>
          )}
          <div className="p-6 space-y-4">
            <div>
