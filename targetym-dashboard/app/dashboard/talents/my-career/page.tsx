...
export default function MyCareerPage() {
  ...
  const estimatedDelay = (() => {
    if (career.eligibility_status === 'eligible') return { label: tp.eligibleNow, color: 'text-green-600', urgent: true };
    if (!career.next_level_id) return null;
    if (totalCount === 0) return null;
    const remaining = totalCount - validatedCount;
    if (remaining === 0) return { label: tp.waitingHR, color: 'text-blue-600', urgent: false };
    if (validatedCount === 0 || !career.level_start_date) {
      ...
      return { label: tp.estimateMonths(est), color: 'text-gray-500', urgent: false };
    }
    ...
    if (monthsLeft <= 1) return { label: tp.lessThanOneMonth, color: 'text-green-600', urgent: true };
    if (monthsLeft <= 3) return { label: tp.aboutMonths(monthsLeft), color: 'text-blue-600', urgent: false };
    return { label: tp.aboutMonths(monthsLeft), color: 'text-orange-500', urgent: false };
  })();

  return (
    <>
      <Header title={tp.title} subtitle={career.path_name} />
      <main className="flex-1 p-6 overflow-auto bg-gray-50 space-y-6">

        {/* ── Timeline des niveaux ── */}
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
          <h3 className="font-semibold text-gray-900 mb-6">{tp.progressionInPath}</h3>
              ...
              return (
                <div key={level.id} className="flex items-center flex-1">
                  <div className="flex flex-col items-center flex-shrink-0">
                    ...
                    {isCurrent && <span className="mt-1 text-[10px] bg-primary-100 text-primary-600 px-1.5 py-0.5 rounded-full font-medium">{tp.current}</span>}
                    {isNext && <span className="mt-1 text-[10px] bg-primary-50 text-primary-500 px-1.5 py-0.5 rounded-full">{tp.next}</span>}
                  </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6" data-tour="my-evaluation">
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            <h3 className="font-semibold text-gray-900 mb-4">{tp.myStatus}</h3>
            <span className={`inline-flex items-center px-3 py-1.5 rounded-full text-sm font-medium ${elig.color}`}>
              {elig.label}
            </span>
            <div className="mt-5">
              <div className="flex justify-between text-sm mb-2">
                <span className="text-gray-600">{tp.validatedCompetencies}</span>
            ...
            {career.level_start_date && (
              <div className="mt-4 flex items-center gap-1.5 text-xs text-gray-400">
                ...
                {tp.onLevelSince(formatDate(career.level_start_date))}
              </div>
            )}
          ...
          <div className="lg:col-span-2 bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            <div className="flex items-start justify-between mb-4">
              <div>
                <h3 className="font-semibold text-gray-900">{tp.nextStep}</h3>
                {career.next_level_title ? (
                  ...
                  <p className="text-sm text-gray-500 mt-1">{tp.maxLevelReached} 🎉</p>
                )}
              </div>
              <Link
              ...
              >
                ...
                {career.eligibility_status === 'eligible' && !hasPendingPromotion
                  ? tp.requestMyPromotion
                  : hasPendingPromotion ? tp.requestInProgress : tp.myPromotions}
              </Link>
            </div>
            ...
            {career.promotion_factors?.length > 0 && (
              <div className="border-t border-gray-100 pt-4">
                <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">{tp.additionalCriteria}</p>
                  ...
                  {career.promotion_factors.map((f: any) => (
                    <div key={f.id} className="flex items-center gap-2 text-sm text-gray-600">
                      ...
                      <span className="ml-auto text-xs text-gray-400">
                        {f.factor_type === 'auto' ? tp.automatic : f.factor_type === 'committee' ? tp.committee : tp.nPlusOne}
                      </span>
                    </div>
                  ))}
                </div>
        ...
        {career.next_level_title && pendingCompetencies.length > 0 && (
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-semibold text-gray-900">{tp.toReach(career.next_level_title)}</h3>
              <span className="ml-auto text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">
                {tp.remainingCompetencies(pendingCompetencies.length)}
              </span>
            </div>
              ...
              {pendingTrainings.length > 0 && (
                <div className="bg-primary-50 border border-primary-100 rounded-xl p-4">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <p className="text-sm font-medium text-primary-800">{tp.trainingsToComplete}</p>
                  </div>
              ...
              {missingScores.length > 0 && (
                <div className="bg-orange-50 border border-orange-100 rounded-xl p-4">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <p className="text-sm font-medium text-orange-800">{tp.scoresToImprove}</p>
                  </div>
                  <div className="space-y-2">
                    {missingScores.map((item, i) => (
                      <div key={i} className="flex items-center justify-between bg-white rounded-lg px-3 py-2 border border-orange-100">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-gray-400">— {item.type === 'performance' ? tp.performance : tp.attitudes}</span>
                        </div>
                        <div className="flex items-center gap-1.5 text-xs">
                          <span className="text-orange-600 font-medium">{item.current != null ? `${item.current.toFixed(0)}%` : 'N/A'}</span>
                          <ChevronRight className="w-3 h-3 text-gray-400" />
                          <span className="text-gray-500">{tp.required(item.required)}</span>
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        {attitudeScores?.scores?.length > 0 && (
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
            <div className="flex items-center gap-2 mb-5">
              ...
              <h3 className="font-semibold text-gray-900">{tp.myAttitudeScores}</h3>
              <span className="ml-auto text-xs text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full">
                {tp.globalScore} : <span className={`font-semibold ${attitudeScores.global_score >= 95 ? 'text-green-600' : attitudeScores.global_score >= 60 ? 'text-orange-500' : 'text-red-500'}`}>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
                ...
                return (
                  <div key={s.attitude_id} className={`rounded-xl border p-4 ${ok ? 'bg-green-50 border-green-200' : mid ? 'bg-orange-50 border-orange-200' : 'bg-red-50 border-red-200'}`}>
                    ...
                    <div className="flex items-center gap-3 text-xs text-gray-500">
                      ...
                      <span className="ml-auto">{tp.reviews(s.total_feedbacks)}</span>
                    </div>
                  </div>
                );
              })}
              {attitudeScores.scores.filter((s: any) => s.total_feedbacks === 0).length > 0 && (
                <div className="sm:col-span-2 lg:col-span-3 text-center py-3 text-xs text-gray-400 border border-dashed border-gray-200 rounded-xl">
                  {tp.attitudesNoData(attitudeScores.scores.filter((s: any) => s.total_feedbacks === 0).length)}
                </div>
              )}
            </div>
          </div>
        ...
        {totalCount > 0 && (
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6" data-tour="development-plan">
            <h3 className="font-semibold text-gray-900 mb-4">
              {tp.competencyDetail} — "{career.next_level_title || career.current_level_title}"
            </h3>
            <div className="space-y-3">
                  ...
                  <div key={comp.id} className={`border rounded-xl p-4
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          {comp.is_mandatory && (
                            <span className="text-[10px] bg-orange-50 text-orange-600 px-1.5 py-0.5 rounded font-medium">{tp.mandatory}</span>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      <div className={`p-3 rounded-lg border ${comp.theoretical_status === 'validated' ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-100'}`}>
                        <div className="flex items-center gap-1.5 mb-1">
                          ...
                          <p className="text-xs font-medium text-gray-600">{tp.training}</p>
                        </div>
                        <p className={`text-xs ${comp.theoretical_status === 'validated' ? 'text-green-600 font-medium' : 'text-gray-500'}`}>
                          {comp.theoretical_status === 'validated' ? `${tp.completed} ✓` : tp.waiting}
                        </p>
                        {comp.post_training_score != null && (
                          <p className="text-xs text-gray-400 mt-0.5">{tp.score} : {comp.post_training_score.toFixed(0)}%</p>
                        )}
                      </div>
                      <div className={`p-3 rounded-lg border ${perfOk ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-100'}`}>
                        <div className="flex items-center gap-1.5 mb-1">
                          ...
                          <p className="text-xs font-medium text-gray-600">{tp.performance}</p>
                        </div>
                      ...
                      <div className={`p-3 rounded-lg border ${attOk ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-100'}`}>
                        <div className="flex items-center gap-1.5 mb-1">
                          ...
                          <p className="text-xs font-medium text-gray-600">{tp.attitudes}</p>
                        </div>
                    ...
                    {comp.required_trainings?.length > 0 && (
                      <div className="mt-3 pt-3 border-t border-gray-100">
                        <p className="text-xs text-gray-400 mb-1.5">{tp.requiredTrainings}</p>
