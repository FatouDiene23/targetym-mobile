...
export async function getDepartmentsTree(subsidiaryTenantId?: number): Promise<DepartmentWithChildren[]> {
  const url = subsidiaryTenantId
    ? `${API_URL}/api/departments/tree?subsidiary_tenant_id=${subsidiaryTenantId}`
    : `${API_URL}/api/departments/tree`;
  const response = await fetchWithAuth(url, {});
...
export interface FixTransferredUsersResult {
  fixed: number;
  dry_run: boolean;
  message: string;
  details: { user_id: number; email: string; old_tenant_id: number; new_tenant_id: number; old_employee_id: number; new_employee_id: number; transfer_id: number }[];
}

/** Migre les comptes User des anciens transferts inter-filiales (super admin) */
export async function fixTransferredUsers(dryRun = false): Promise<FixTransferredUsersResult> {
  const response = await fetchWithAuth(`${API_URL}/api/platform/fix-transferred-users?dry_run=${dryRun}`, {
    method: 'POST',
  });
  if (!response.ok) {
    const error = await parseApiError(response);
...
export async function getSOSAlerts(params?: {
  ...
  if (params?.subsidiary_tenant_id) q.set('subsidiary_tenant_id', String(params.subsidiary_tenant_id));
  if (params?.skip) q.set('skip', String(params.skip));
  if (params?.limit) q.set('limit', String(params.limit));
  const response = await fetchWithAuth(`${API_URL}/api/sos?${q.toString()}`);
  if (!response.ok) { const error = await parseApiError(response); throw new Error(error); }
  return response.json();
}
...
export async function updateSOSAlertStatus(
  ...
  const response = await fetchWithAuth(`${API_URL}/api/sos/${alertId}/status`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
